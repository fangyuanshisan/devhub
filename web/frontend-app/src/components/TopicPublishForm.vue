...
const props = defineProps<{ defaultCommunity?: string; defaultContentType?: string }>();
...
const form = reactive({
  ...
  category_id: 0,
  content_type: initialContentType(props.defaultContentType),
  title: '',
  summary: '',
  content: '',
  tags: [] as string[],
...
function initialContentType(defaultType = 'article') {
  const fallback = normalizeContentType(defaultType || 'article');
  if (typeof window === 'undefined') return contentTypeOptions.some((item) => item.value === fallback) ? fallback : 'article';
  const raw = new URLSearchParams(window.location.search).get('type') || new URLSearchParams(window.location.search).get('content_type') || fallback;
