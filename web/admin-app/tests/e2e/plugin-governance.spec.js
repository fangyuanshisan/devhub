...
test.describe('plugin governance center', () => {
  ...
  test('shows health summary and supports manifest validate dry-run and install dialogs', async ({ page, request }) => {
    ...
    try {
      await page.goto('/admin-next/plugins/list');
      await page.getByRole('button', { name: /健康摘要/ }).click();
  ...
  test('opens plugin detail tabs and shows schema validation errors', async ({ page }) => {
    ...
    const detailTabs = page.getByTestId('plugin-detail-tabs');
    for (const tabName of ['概览', '能力', '运行记录', '配置', '技术详情']) {
      await detailTabs.getByRole('tab', { name: tabName }).click();
      await expect(detailTabs.getByRole('tab', { name: tabName })).toHaveAttribute('aria-selected', 'true');
    }

    await detailTabs.getByRole('tab', { name: '能力' }).click();
    await expect(page.getByTestId('plugin-capabilities-summary')).toBeVisible();
    await expect(page.getByTestId('plugin-capabilities-panel')).toContainText('前端挂载');

    await detailTabs.getByRole('tab', { name: '配置' }).click();
    const globalPanel = page.getByTestId('plugin-global-config-panel');
    await expect(globalPanel).toBeVisible();
    await expect(globalPanel).toContainText('配置模型');
    await expect(globalPanel.getByTestId('plugin-json-editor')).toBeVisible();
    ...
    await expect(page.getByTestId('schema-error-box')).toContainText(/required|必填字段/);
  ...
  test('archives plugin and shows archived state with restore entry', async ({ page, request }) => {
    try {
      ...
      await qaRow.getByRole('button', { name: '详情' }).first().click();
      await expect(page.getByTestId('plugin-detail-drawer')).toBeVisible();
      await page.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '概览' }).click();
      await expect(page.getByText('归档时间')).toBeVisible();
      await closePluginDetailDrawer(page);
  ...
  test('shows impact before global disable and blocks community enable when globally disabled', async ({ page, request }) => {
    try {
      ...
      const communityPluginDrawer = page.getByTestId('community-plugin-drawer');
      await expect(communityPluginDrawer).toBeVisible();
      await expect(communityPluginDrawer.getByText('该插件已被全局禁用').first()).toBeVisible();
    } finally {
      await ensurePluginEnabled(request, 'qa');
    }
  });
  ...
  test('renders enhanced config_schema auto form with grouping, enum, boolean, number, required and sensitive masking', async ({ page, request }) => {
    ...
    try {
      ...
      await page.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '配置' }).click();
  ...
  test('opens generic plugin content page with filters', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/admin-next\/plugins\/overview\?tab=content/);
  });
  ...
  test('failed migration blocks enablement until retry succeeds', async ({ page, request }) => {
    ...
    try {
      ...
      const drawer = page.getByTestId('plugin-detail-drawer');
      await expect(drawer).toBeVisible();
      await drawer.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '运行记录' }).click();
      await expect(drawer.getByTestId('plugin-runtime-panel').getByText(/迁移|失败|异常/).first()).toBeVisible();
      // UI retry button / technical migration panel were收敛到技术详情与服务端 API，保留 API 级验证。
  ...
  test('hook failure injection records blocking and non-blocking failures', async ({ page, request }) => {
    ...
    try {
      ...
      await expect(drawer).toBeVisible();
      await drawer.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '运行记录' }).click();
      await expect(drawer.getByTestId('plugin-runtime-panel').getByText(/Hook|失败|异常/).first()).toBeVisible();
    } finally {
  ...
  test('shows plugin health states and filters plugin audit logs', async ({ page, request }) => {
    ...
    try {
      ...
      await expect(drawer).toBeVisible();
      await drawer.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '运行记录' }).click();
      await expect(drawer.getByTestId('plugin-runtime-panel').getByText(/运行状态由.*Hook/)).toBeVisible();
      await closePluginDetailDrawer(page);
      ...
      await drawer.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '运行记录' }).click();
      await expect(drawer.getByTestId('plugin-runtime-panel').getByText('异常').first()).toBeVisible();
      await expect(drawer.getByTestId('plugin-runtime-panel').getByText(/failed migration|失败迁移|迁移/).first()).toBeVisible();
      await retryPluginMigration(request, 'qa', 'qa_questions');
      ...
      drawer = page.getByTestId('plugin-detail-drawer');
      await drawer.getByTestId('plugin-detail-tabs').getByRole('tab', { name: '运行记录' }).click();
      const runtimePanel = drawer.getByTestId('plugin-runtime-panel');
      await expect(runtimePanel.getByText(/运行状态由.*Hook/)).toBeVisible();
      await expect(runtimePanel.getByRole('button', { name: '查看 Hook 排障' })).toBeVisible();
      await expect(runtimePanel.getByRole('button', { name: '查看审计日志' })).toBeVisible();

      await runtimePanel.getByRole('button', { name: '查看审计日志' }).click();
      await expect(page).toHaveURL(/\/admin-next\/plugins\/runtime/);
      await expect(page.getByTestId('plugin-runtime-domain')).toBeVisible();
      await expect(page.getByTestId('plugin-runtime-domain-tabs').getByRole('tab', { name: '审计日志' })).toHaveAttribute('aria-selected', 'true');
      await expect(page.getByTestId('plugin-audit-page')).toBeVisible();
      await page.getByTestId('plugin-audit-open-hooks').click();
      await expect(page).toHaveURL(/\/admin-next\/audit-logs/);
...
async function closePluginDetailDrawer(page) {
  const drawer = page.getByTestId('plugin-detail-drawer');
