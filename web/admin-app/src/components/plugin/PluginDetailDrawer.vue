      ...
      <div class="drawer-content">
        ...
        <div class="hero-left">
          <div class="hero-title">
            ...
            <AdminStatusTag :value="plugin.status" />
            <AdminStatusTag :value="plugin.health?.status" />
        ...
        <el-tab-pane :label="t('plugin.tabs.overview')" name="overview">
          ...
          <section class="dh-admin-metrics mb" data-testid="plugin-detail-conclusion">
            <AdminMetricCard label="插件当前状态" :value="pluginStatusLabel(plugin.status)" :status="plugin.status" />
            <AdminMetricCard label="当前健康" :value="pluginHealthLabel(plugin.health?.status)" :status="plugin.health?.status" />
            <AdminMetricCard label="当前版本" :value="safePlugin.version || '-'" help="安装版本与运行态声明版本" />
            <AdminMetricCard label="当前风险" :value="currentRiskText" :status="currentRiskStatus" />
            <AdminMetricCard label="下一步建议" :value="nextActionText" help="优先处理配置、依赖、迁移或 external_service" />
          </section>
          ...
          <section v-if="hasExternalServiceCapability" class="compact-section mt" data-testid="plugin-external-service-overview">
            <div class="section-head">
              <div>
                <h4>external_service 运行配置</h4>
                <p>这里展示 Webhook 投递实际使用的运行配置，不等同于插件全局配置 config_json。</p>
              </div>
              <div class="section-actions">
                <el-button size="small" type="primary" plain data-testid="plugin-external-service-config-shortcut" @click="tab = 'runtime'">配置 external_service</el-button>
                <el-button size="small" plain @click="openExternalServiceExecutions">查看历史记录</el-button>
              </div>
            </div>
            <el-descriptions :column="2" border size="small">
              <el-descriptions-item label="当前 endpoint_url" :span="2">{{ externalService?.endpoint_url || '尚未配置' }}</el-descriptions-item>
              <el-descriptions-item label="health_check_path">{{ externalService?.health_check_path || '/health' }}</el-descriptions-item>
              <el-descriptions-item label="timeout_ms">{{ externalService?.timeout_ms || 3000 }}</el-descriptions-item>
              <el-descriptions-item label="failure_policy">{{ externalServiceFailurePolicyLabel(externalService?.failure_policy) }}</el-descriptions-item>
              <el-descriptions-item label="enabled">{{ externalService?.enabled === false ? '否' : '是' }}</el-descriptions-item>
              <el-descriptions-item label="当前健康">
              ...
              <el-descriptions-item label="最近健康检查">{{ externalService?.last_checked_at || '-' }}</el-descriptions-item>
              <el-descriptions-item label="最近成功">{{ plugin.health?.external_service_last_success_at || externalService?.last_success_at || '-' }}</el-descriptions-item>
              <el-descriptions-item label="最近成功 execution_id">{{ plugin.health?.external_service_recent_success_execution_id || '-' }}</el-descriptions-item>
              <el-descriptions-item label="最近失败">{{ plugin.health?.external_service_last_failure_at || externalService?.last_failure_at || '-' }}</el-descriptions-item>
              <el-descriptions-item label="最近失败 execution_id">{{ plugin.health?.external_service_recent_failure_execution_id || '-' }}</el-descriptions-item>
              <el-descriptions-item label="历史失败计数">24h {{ plugin.health?.external_service_failure_count_24h ?? 0 }} / 7 天 {{ plugin.health?.external_service_failure_count_7d ?? 0 }}</el-descriptions-item>
              <el-descriptions-item label="最近失败摘要" :span="2">{{ timestampedExternalServiceError }}</el-descriptions-item>
            </el-descriptions>
            <el-alert
              v-if="externalServiceHealthyWithHistoricalFailure"
              class="mt"
              type="success"
              show-icon
              :closable="false"
              title="当前 external_service 已恢复 healthy；下方最近失败是历史记录，不代表当前 endpoint 仍错误。"
            />
          </section>
          <el-alert
            class="mt"
        ...
        <el-tab-pane label="能力" name="capabilities" lazy>
          <div data-testid="plugin-capabilities-panel">
          ...
          />
          <section class="summary-grid mb" data-testid="plugin-capabilities-summary">
        ...
        <el-tab-pane label="运行记录" name="runtime" lazy>
          <div data-testid="plugin-runtime-panel">
          ...
          <section v-if="hasExternalServiceCapability" class="compact-section mt" data-testid="plugin-external-service-runtime">
            <div class="section-head">
              <div>
                <h4>外部服务配置</h4>
            ...
            <el-alert
              v-if="externalServiceDockerHint"
              class="mb"
              type="warning"
              show-icon
              :closable="false"
              title="DevHub 后端在容器中运行时，127.0.0.1 指向容器自身。若 Webhook 接收端运行在宿主机，请使用 host.docker.internal、Docker host gateway 地址，或宿主机局域网 IP。"
            />
            <el-alert
              class="mb"
              type="info"
              show-icon
              :closable="false"
              :title="externalServiceHTTPPolicyText"
            />
            <el-alert
              v-for="item in externalServiceDiagnostics"
              ...
              :title="item.message"
            >
              <template #default>{{ item.suggestion || '-' }}</template>
            </el-alert>
            <el-form class="external-service-form" label-width="130px" size="small" data-testid="plugin-external-service-form">
              ...
              <el-form-item label="服务地址">
                <el-input v-model="externalServiceForm.endpoint_url" placeholder="https://example.com 或 http://127.0.0.1:18081" data-testid="plugin-external-service-endpoint" />
              </el-form-item>
            ...
            <el-descriptions :column="2" border size="small" class="mt">
              ...
              <el-descriptions-item label="健康检查路径">{{ externalService?.health_check_path || '/health' }}</el-descriptions-item>
              ...
              <el-descriptions-item label="启用投递">{{ externalService?.enabled === false ? '否' : '是' }}</el-descriptions-item>
              ...
              <el-descriptions-item label="Token 引用" :span="2">
                <span v-if="externalService?.token_ref" class="masked-ref" :title="externalService.token_ref">
                  <span class="mono">{{ maskRef(externalService.token_ref) }}</span>
                  <el-button link type="primary" size="small" @click="copyText(externalService.token_ref)">复制</el-button>
                </span>
                <span v-else>-</span>
              </el-descriptions-item>
              <el-descriptions-item label="Token 状态">{{ externalService?.token_secret?.status || (externalService?.token_ref ? 'unknown' : '-') }}</el-descriptions-item>
              <el-descriptions-item label="Token key_id">{{ externalService?.token_secret?.key_id || '-' }}</el-descriptions-item>
              <el-descriptions-item label="Token 最近使用">{{ externalService?.token_secret?.last_used_at || '-' }}</el-descriptions-item>
              <el-descriptions-item label="Token 轮换时间">{{ externalService?.token_secret?.rotated_at || '-' }}</el-descriptions-item>
              ...
              <el-descriptions-item label="最近成功">{{ externalService?.last_success_at || '-' }}</el-descriptions-item>
        ...
        <el-tab-pane :label="plugin.code === 'official_announcement' ? '公告配置' : t('plugin.tabs.config')" name="config" lazy>
          ...
          <section class="config-card" data-testid="plugin-global-config-panel">
            ...
            <section
              v-if="suspiciousExternalServiceConfigFields.length"
              class="external-service-config-warning mb"
              data-testid="plugin-global-config-external-service-warning"
            >
              <div class="warning-main">
                <div>
                  <h4>这些字段不会自动影响 Webhook 投递</h4>
                  <p>
                    检测到全局配置 config_json 中包含 {{ suspiciousExternalServiceConfigFields.join('、') }}。
                    这些字段属于插件全局配置，不会自动作为 Webhook 投递运行配置。请到 external_service 配置中修改。
                  </p>
                </div>
                <div class="warning-actions">
                  <el-button size="small" type="warning" @click="tab = 'runtime'">配置 external_service</el-button>
                  <el-button size="small" plain @click="openExternalServiceExecutions">查看投递记录</el-button>
                </div>
              </div>
              <div class="external-service-compare">
                <div v-for="row in suspiciousExternalServiceRows" :key="row.key" class="external-service-compare-row">
                  <span>{{ row.label }}</span>
                  <strong>{{ row.globalValue }}</strong>
                  <small>运行配置：{{ row.runtimeValue }}</small>
                </div>
              </div>
            </section>
            <PluginConfigEditor
              ...
              :show-preview="!suspiciousExternalServiceConfigFields.length"
...
import { AdminMetricCard, AdminStatusTag } from '@/components/admin';
...
import { t } from '@/i18n';
import { auditActionLabel, genericStatusLabel, maskRef, maturityLabel, migrationStatusLabel, pluginHealthLabel, pluginStatusLabel } from '@/i18n/formatters';
...
const hasExternalServiceCapability = computed(() => {
  if (props.plugin?.external_service) return true;
  return (props.plugin?.hooks || []).some((hook) => String(hook?.service_type || '').trim() === 'external_service');
});
const suspiciousExternalServiceConfigFields = computed(() => {
  const cfg = editableConfig.value && typeof editableConfig.value === 'object' ? editableConfig.value : {};
  const fields = ['endpoint_url', 'health_check_path', 'token', 'timeout_ms', 'failure_policy'];
  return fields.filter((field) => Object.prototype.hasOwnProperty.call(cfg, field));
});
const suspiciousExternalServiceRows = computed(() => {
  const cfg = editableConfig.value && typeof editableConfig.value === 'object' ? editableConfig.value : {};
  const runtime = externalService.value || {};
  const labels = {
  ...
  return suspiciousExternalServiceConfigFields.value.map((key) => ({
    key,
    label: labels[key] || key,
    globalValue: externalServiceDisplayValue(cfg[key], key),
    runtimeValue: externalServiceDisplayValue(runtime[key], key),
  }));
});
const externalServiceDiagnostics = computed(() => Array.isArray(externalService.value?.diagnostics) ? externalService.value.diagnostics : []);
const currentRiskStatus = computed(() => {
  const health = String(props.plugin?.health?.status || '').toLowerCase();
  const status = String(props.plugin?.status || '').toLowerCase();
  if (status === 'archived' || health === 'error' || health === 'failed') return 'error';
  if (health === 'warning' || status === 'disabled') return 'warning';
  if (status === 'enabled' && (health === 'healthy' || health === 'success')) return 'healthy';
  return 'unknown';
});
const currentRiskText = computed(() => {
  if (currentRiskStatus.value === 'error') return '需要处理';
  if (currentRiskStatus.value === 'warning') return '需要关注';
  if (currentRiskStatus.value === 'healthy') return '无需处理';
  return '待确认';
});
const nextActionText = computed(() => {
  if (props.plugin?.health?.suggested_action) return props.plugin.health.suggested_action;
  if (props.plugin?.status === 'disabled') return '确认配置后启用';
  if (props.plugin?.status === 'archived') return '如需恢复请走恢复入口';
  return '查看配置和运行记录';
});
const externalServiceDockerHint = computed(() => {
  const endpoint = String(externalServiceForm.endpoint_url || externalService.value?.endpoint_url || '').trim().toLowerCase();
  return endpoint.startsWith('http://127.0.0.1:') || endpoint.startsWith('http://localhost:');
});
const externalServiceHTTPPolicyText = computed(() => {
  const origins = externalService.value?.http_policy?.allowlist_origins || [];
  const allowlistText = origins.length ? `当前 allowlist：${origins.join('、')}` : '当前未读取到非 localhost HTTP allowlist。';
  return `HTTPS 默认允许；localhost / 127.0.0.1 / ::1 HTTP 默认允许；非 localhost HTTP 需要 DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST。${allowlistText} 示例：DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST=http://172.17.0.1:18081 ./dev.sh restart --no-build`;
});
const timestampedExternalServiceError = computed(() => {
  const at = externalService.value?.last_failure_at || props.plugin?.health?.external_service_last_failure_at;
  const msg = externalService.value?.last_error_message || props.plugin?.health?.external_service_recent_error || '';
  if (!msg) return '-';
  return at ? `${at}：${msg}` : msg;
});
const externalServiceHealthyWithHistoricalFailure = computed(() => {
  const status = String(externalService.value?.last_health_status || externalService.value?.status || '').trim();
  return status === 'healthy' && Boolean(externalService.value?.last_failure_at || externalService.value?.last_error_message);
});
...
function externalServiceDisplayValue(value, key) {
  if (key === 'token') {
    if (externalService.value?.token_ref && value === undefined) return '已配置 token_ref，不回显';
    return value ? '已填写，不回显' : '-';
  }
...
async function saveExternalServiceConfig() {
  ...
  try {
    const saved = await updatePluginExternalService(p.code, externalServicePayload());
    externalServiceForm.token = '';
    ElMessage.success(`external_service 配置已保存，当前 endpoint：${saved?.endpoint_url || endpoint}。请执行健康检查确认新配置。`);
    emit('refresh');
  } catch (e) {
...
async function runExternalServiceHealthCheck() {
  ...
  try {
    ...
    ElMessage.error(formatAPIError(e, '健康检查失败，请稍后重试'));
  } finally {
...
function formatAPIError(e, fallback) {
  ...
  const diagnosticText = diagnostics
    .map((item) => [item?.message ? `诊断：${item.message}` : '', item?.suggestion ? `处理：${item.suggestion}` : ''].filter(Boolean).join(' '))
    .filter(Boolean)
    .join(' ');
  return [code ? `[${code}]` : '', message, suggestion ? `建议：${suggestion}` : '', diagnosticText].filter(Boolean).join(' ');
}
...
.external-service-config-warning {
  padding: 14px;
...
.external-service-compare {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 8px;
  margin-top: 12px;
}
.external-service-compare-row {
  padding: 10px;
  border: 1px solid #fde68a;
  border-radius: 8px;
  background: #fff;
}
.external-service-compare-row span,
.external-service-compare-row small {
  display: block;
  color: #92400e;
  font-size: 12px;
}
.external-service-compare-row strong {
  display: block;
  margin: 4px 0;
  color: #0f172a;
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
