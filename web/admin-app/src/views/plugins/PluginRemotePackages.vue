<template>
  <section class="plugin-remote-packages" data-testid="plugin-remote-packages-page">
    <header class="page-head">
      <div>
        <h2>远程插件包</h2>
        <p class="muted">只读治理：下载到受控 staging，进行 sha256、预检、兼容性检查与 detached signature 验签；不会安装、启用或执行插件代码。</p>
      </div>
    ...
    <section v-if="activeTab === 'downloads'" class="card" data-testid="remote-packages-downloads">
      <div class="card-head">
        <div>
          <h3>下载到 staging</h3>
          <p class="muted">仅允许 https 与 .zip/.tar.gz/.tgz（默认 20MB）；signature_url 仅允许 https + .json（默认 64KB）。</p>
        </div>
        <el-button :loading="downloading" type="primary" data-testid="download-to-staging" @click="submitDownload">下载到 staging</el-button>
      </div>
      ...
      <el-form class="download-form" :model="downloadForm" label-width="110px">
        ...
        <el-form-item label="signature_url">
          <el-input
      ...
      <el-table v-loading="loading" :data="stagingItems" data-testid="staging-list" border>
        ...
        <el-table-column prop="plugin_code" label="code" min-width="130" />
        ...
        <el-table-column prop="staging_path" label="staging" min-width="240" show-overflow-tooltip />
      ...
      <PluginEmptyState v-if="!loading && !stagingItems.length" testid="staging-empty-state" description="暂无 staging 下载记录" />
    </section>
    ...
    <section v-else-if="activeTab === 'compat'" class="card" data-testid="remote-packages-compat">
      <div class="card-head">
        <div>
          <h3>依赖 / 兼容性检查</h3>
          <p class="muted">只接受 precheck passed 的记录；只做检查，不安装、不启用、不注册任何声明。</p>
        </div>
      ...
      <el-form class="download-form" :model="compatForm" label-width="110px">
        ...
        <el-form-item>
          ...
          <span class="muted">检查结果会落库，后续安装/升级必须依赖 can_install=true。</span>
        </el-form-item>
      </el-form>
      ...
      <el-table v-loading="compatLoading" :data="compatItems" data-testid="compat-list" border>
        ...
        <el-table-column prop="can_install" label="can_install" width="110" />
        <el-table-column prop="core_version" label="core" width="100" />
        <el-table-column prop="compatible_core_version" label="compatible" min-width="170" show-overflow-tooltip />
        <el-table-column prop="finished_at" label="finished_at" width="170" />
      </el-table>
    ...
    <section v-else class="card" data-testid="remote-packages-signatures">
      <div class="card-head">
        <div>
          ...
          <p class="muted">对 precheck passed 的解压目录执行 devhub-signature.json 验签；verified 才能进入默认安装/升级链路。</p>
        </div>
      ...
      <el-form class="download-form" :model="signatureForm" label-width="110px">
        ...
        <el-form-item>
          ...
          <span class="muted">不会安装、不会启用、不会执行插件代码；签名文件来自 signature_url 或包内 devhub-signature.json。</span>
        </el-form-item>
      </el-form>
      ...
      <el-table v-loading="signatureLoading" :data="signatureItems" data-testid="signature-list" border>
        ...
        <el-table-column prop="publisher_id" label="publisher" min-width="150" show-overflow-tooltip />
        <el-table-column prop="key_id" label="key_id" min-width="160" show-overflow-tooltip />
        <el-table-column prop="verified_at" label="verified_at" width="170" />
        <el-table-column label="操作" width="110" fixed="right">
          <template #default="{ row }">
...
async function fetchStaging() {
  ...
  try {
    ...
    ElMessage.error(error?.message || 'staging 获取失败');
  } finally {
    loading.value = false;
  }
}
