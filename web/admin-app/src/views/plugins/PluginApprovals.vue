  ...
  <section class="plugin-page" data-testid="plugin-approvals-page">
    <div class="plugin-page-header">
      <div>
        ...
        <h2>审批中心</h2>
        <p class="muted">插件安装/升级等高风险操作的审批与执行入口。审批通过不等于绕过后端校验，执行时仍会重新 dry-run。</p>
      </div>
    ...
    <div class="filter-panel mb" data-testid="plugin-approvals-filters">
      ...
      <div class="filter-actions" style="gap: 10px">
        <el-input v-model="qPluginCode" placeholder="plugin_code" clearable style="max-width: 220px" />
        <el-select v-model="qAction" placeholder="action" clearable style="width: 140px">
          <el-option label="install" value="install" />
          <el-option label="upgrade" value="upgrade" />
        </el-select>
        <el-select v-model="qStatus" placeholder="status" clearable style="width: 160px">
    ...
    <el-card shadow="never" data-testid="plugin-approvals-card">
      ...
      <el-table :data="items" size="small" stripe data-testid="plugin-approvals-table">
        ...
        <el-table-column prop="plugin_code" label="plugin" min-width="180">
        ...
        <el-table-column prop="risk_level" label="risk" width="120">
          <template #default="{ row }">
            <el-tag :type="riskType(row.risk_level)" effect="plain">{{ row.risk_level || '-' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="checksum_status" label="checksum" width="120">
          <template #default="{ row }">
            <el-tag :type="checksumType(row.checksum_status)" effect="plain">{{ row.checksum_status || '-' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="requested_at" label="requested_at" width="170" />
        <el-table-column prop="requested_by_name" label="requested_by" width="150" />
        <el-table-column prop="reviewed_at" label="reviewed_at" width="170" />
        <el-table-column prop="reviewed_by_name" label="reviewed_by" width="150" />
    ...
    <el-drawer v-model="detailOpen" size="720px" data-testid="plugin-approval-detail-drawer">
      <template #header>
        <div style="display: flex; gap: 10px; align-items: center">
          ...
          <el-tag v-if="detail?.status" :type="statusType(detail.status)" effect="plain">{{ detail.status }}</el-tag>
          <el-tag v-if="detail?.package_risk_level" :type="riskType(detail.package_risk_level)" effect="plain">risk={{ detail.package_risk_level }}</el-tag>
        </div>
      </template>
      ...
      <el-descriptions v-if="detail" :column="2" border class="mb" data-testid="plugin-approval-detail-info">
        ...
        <el-descriptions-item label="plugin_name">{{ detail.plugin_name || '-' }}</el-descriptions-item>
        <el-descriptions-item label="package_path">{{ detail.package_path || '-' }}</el-descriptions-item>
        <el-descriptions-item label="current_version">{{ detail.current_version || '-' }}</el-descriptions-item>
        <el-descriptions-item label="target_version">{{ detail.target_version || '-' }}</el-descriptions-item>
        <el-descriptions-item label="checksum">{{ detail.package_checksum_status || '-' }}</el-descriptions-item>
        <el-descriptions-item label="requested_at">{{ detail.requested_at || '-' }}</el-descriptions-item>
        <el-descriptions-item label="requested_by">{{ detail.requested_by_name || '-' }}</el-descriptions-item>
        <el-descriptions-item label="reviewed_at">{{ detail.reviewed_at || '-' }}</el-descriptions-item>
        <el-descriptions-item label="reviewed_by">{{ detail.reviewed_by_name || '-' }}</el-descriptions-item>
      </el-descriptions>
    ...
    <el-dialog v-model="reviewOpen" width="560px" data-testid="plugin-approval-review-dialog">
      ...
      <el-input v-model="reviewComment" type="textarea" :rows="4" placeholder="comment（拒绝必填）" data-testid="plugin-approval-review-comment" />
      <template #footer>
...
async function executeRequest() {
  if (!detail.value) return;
  await ElMessageBox.confirm('确认执行该审批？执行前会重新 dry-run 校验；不会执行第三方代码/SQL/前端资产。', '执行确认', { type: 'warning' });
  try {
