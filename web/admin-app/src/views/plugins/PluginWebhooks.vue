<template>
  <section class="plugin-webhooks" data-testid="plugin-webhooks-page">
    <AdminPageHeader
      title="Webhook 与回调"
      description="管理 DevHub 到外部服务的投递、重试、熔断和密钥，也管理外部插件服务回调 Core 的 Token 与请求记录。"
      testid="webhook-page-header"
    >
      <template #actions>
        <el-button
          size="small"
    ...
    <PluginErrorAlert :message="pluginOptionsError" />

    <el-tabs v-model="tab" class="page-tabs" data-testid="webhook-tabs">
      <el-tab-pane label="总览" name="overview">
        <div class="webhook-overview-grid">
          <AdminSectionCard title="投递概况" description="当前健康、历史失败和待处理异常分开查看，避免把 skipped / disabled / archived 误判为失败。">
            <div class="dh-admin-metrics" data-testid="webhook-health-cards">
              <AdminMetricCard label="投递记录" :value="deliveryRows.length" help="最近 non-blocking 投递" clickable @click="tab = 'deliveries'" />
              <AdminMetricCard label="等待重试" :value="retryRows.length" status="warning" help="失败后进入重试队列" clickable @click="tab = 'exceptions'" />
              <AdminMetricCard label="熔断中" :value="openCircuitCount" status="blocked" help="需要先恢复外部服务" clickable @click="tab = 'exceptions'" />
              <AdminMetricCard label="外部服务失败" :value="externalFailureCount" :status="externalFailureCount ? 'warning' : 'healthy'" help="历史失败计数，不等于当前健康" clickable @click="tab = 'external_service'" />
            </div>
          ...
          <AdminSectionCard title="下一步操作" description="优先配置 external_service、查看运行记录，再处理低频密钥和回调治理。">
            <div class="overview-actions">
              ...
              <el-button size="small" type="primary" plain @click="goExternalServiceConfig">配置 external_service</el-button>
      ...
      <el-tab-pane label="投递记录" name="deliveries">
        <PluginFilterBar title="投递记录" tip="记录每次 non_blocking Webhook 投递结果；异常记录会汇总到“异常处理”。" testid="webhook-deliveries-filter">
          ...
          <PluginCodeSelect
            v-model="deliveryFilters.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="选择插件"
            data-testid="webhook-deliveries-plugin-select"
          />
      ...
      <el-tab-pane label="外部服务执行" name="external_service">
        <PluginFilterBar title="external_service 执行记录" tip="查看声明型 external_service Hook 的非阻塞投递结果；失败类记录可手动重试。" testid="external-service-executions-filter">
          ...
          <PluginCodeSelect
            v-model="externalExecutionFilters.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="全部插件"
            data-testid="external-service-plugin-select"
          />
          <el-input v-model="externalExecutionFilters.hook_name" size="small" placeholder="Hook 名称" style="width: 200px" />
          <el-input v-model="externalExecutionFilters.request_id" size="small" placeholder="request_id" style="width: 220px" />
        ...
        <el-alert
          ...
          show-icon
          :closable="false"
          title="重试会重新向外部服务投递事件；请确认接收端具备幂等处理能力。配置 endpoint_url 请进入 external_service 运行配置，不要写在插件全局 config_json。Token、Secret、Authorization Header 不会在执行记录中展示。"
        />
      ...
      <el-tab-pane label="事件通知" name="events">
        <PluginFilterBar title="Webhook 事件" tip="事件记录用于追踪投递链路，不展示敏感 payload 明文" testid="webhook-events-filter">
          ...
          <PluginCodeSelect
            v-model="eventFilters.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="选择插件"
            data-testid="webhook-events-plugin-select"
          />
      ...
      <el-tab-pane label="Webhook 密钥" name="secrets">
        <PluginFilterBar title="Webhook 密钥" tip="密钥明文只会在创建/轮换时展示一次；表格只显示 secret_ref。" testid="webhook-secrets-filter">
          ...
          <PluginCodeSelect
            v-model="secretFilters.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="选择插件"
            data-testid="webhook-secrets-plugin-select"
          />
        ...
        >
          <el-table-column prop="id" label="ID" width="80" />
          <el-table-column prop="plugin_code" label="插件" width="160" />
          <el-table-column label="密钥引用" min-width="220">
            <template #default="{ row }"><MaskedRef :value="row.secret_ref" /></template>
          </el-table-column>
          <el-table-column prop="target_url" label="目标 URL" min-width="260" show-overflow-tooltip />
          <el-table-column label="状态" width="140">
            <template #default="{ row }">
      ...
      <el-tab-pane label="回调 Token" name="callback_tokens">
        <PluginFilterBar title="回调 Token" tip="回调 Token 不等于管理员权限；明文只会在创建/轮换时展示一次。" testid="callback-tokens-filter">
          ...
          <PluginCodeSelect
            v-model="callbackTokenFilters.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="选择插件"
            data-testid="callback-tokens-plugin-select"
          />
        ...
        <el-dialog v-model="createCallbackTokenDialogVisible" title="创建回调 Token" width="620px">
          <el-form :model="createCallbackTokenForm" label-width="120px">
            <el-form-item label="插件编码">
              <PluginCodeSelect
                v-model="createCallbackTokenForm.plugin_code"
                :options="pluginSelectOptions"
                :loading="pluginOptionsLoading"
                placeholder="选择插件"
                size="default"
      ...
      <el-tab-pane label="回调请求" name="callback_requests">
        <PluginFilterBar title="回调请求" tip="插件服务回调受控 Core API 的请求记录（不保存 Token 明文）" testid="callback-requests-filter">
          ...
          <PluginCodeSelect
            v-model="callbackRequestFilters.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="选择插件"
            data-testid="callback-requests-plugin-select"
          />
        ...
        >
          ...
          <el-table-column prop="created_at" label="创建时间" width="170" />
          <el-table-column prop="plugin_code" label="插件" width="160" />
          <el-table-column label="Token 引用" min-width="220">
            <template #default="{ row }"><MaskedRef :value="row.token_ref" /></template>
          </el-table-column>
    ...
    <el-dialog v-model="createSecretDialogVisible" title="创建 Webhook 密钥" width="560px">
      ...
      <el-form label-width="100px">
        <el-form-item label="插件编码">
          <PluginCodeSelect
            v-model="createSecretForm.plugin_code"
            :options="pluginSelectOptions"
            :loading="pluginOptionsLoading"
            placeholder="选择插件"
            size="default"
            data-testid="webhook-secret-form-plugin"
          />
        </el-form-item>
...
import PluginCodeSelect from './components/PluginCodeSelect.vue';
...
import MaskedRef from './components/MaskedRef.vue';
import { AdminMetricCard, AdminPageHeader, AdminSectionCard } from '@/components/admin';

const route = useRoute();
const router = useRouter();

const pluginOptions = ref([]);
const pluginOptionsLoading = ref(false);
const pluginOptionsError = ref('');

function normalizePluginSelectOptions(value) {
  const source = Array.isArray(value?.items) ? value.items : Array.isArray(value) ? value : [];
  return source
    .map((item) => {
      const code = String(item?.code || item?.plugin_code || '').trim();
      if (!code) return null;
      const name = String(item?.name || '').trim();
      const status = String(item?.status || item?.health_status || '').trim();
      const namePart = name && name !== code ? `${name} (${code})` : code;
      return {
        value: code,
        label: status ? `${namePart} · ${status}` : namePart,
      };
    })
    .filter(Boolean)
...
function goExternalServiceConfig() {
  const pluginCode = String(externalExecutionFilters.value.plugin_code || deliveryFilters.value.plugin_code || '').trim();
  router.push({
    path: '/plugins/overview',
    query: {
      tab: 'list',
      plugin_code: pluginCode || undefined,
      detail_tab: 'runtime',
    },
  });
}

const pluginSelectOptions = computed(() => pluginOptions.value);

async function loadPluginOptions() {
  pluginOptionsLoading.value = true;
  pluginOptionsError.value = '';
  try {
    const res = await admin.plugins();
    pluginOptions.value = normalizePluginSelectOptions(res);
  } catch (e) {
    pluginOptions.value = [];
    pluginOptionsError.value = e?.message || '插件列表加载失败';
  } finally {
    pluginOptionsLoading.value = false;
  }
}
...
const externalExecutionFilters = ref({
  ...
  request_id: String(route.query.ext_request_id || ''),
...
const externalFailureCount = computed(() => externalExecutionRows.value.filter((row) => !row.success && !['skipped', 'disabled', 'archived'].includes(String(row.status || '').toLowerCase())).length);
...
function openCreateSecret() {
  // Readiness check: creating Webhook Secret requires startup root keyring (env-only).
  admin.getPluginConfigKeyStatus()
    .then((status) => {
      const ok = String(status?.status || '').toLowerCase() === 'ok';
      if (!ok) {
        const examples = Array.isArray(status?.env_examples) ? status.env_examples : [];
        const msg = (status?.warnings || []).slice(0, 1).join('') || '缺少启动加密密钥，无法创建 Webhook Secret。';
        const hint = examples.length ? `\n\n环境变量示例：\n${examples.join('\n\n')}` : '';
        throw new Error(`${msg}\n\n该密钥只能在启动环境变量中配置，后台不会保存或生成；修改后需重启生效。${hint}`);
      }
      createSecretForm.value = { plugin_code: secretFilters.value.plugin_code || '', target_url: '' };
      createSecretDialogVisible.value = true;
    })
    .catch((e) => {
      secretsError.value = e?.message || String(e || '缺少启动加密密钥');
    });
}
...
async function refreshExternalServiceExecutions() {
  ...
  try {
    const params = {
      ...
      request_id: externalExecutionFilters.value.request_id || '',
    ...
    await router.replace({
      query: {
        ...
        ext_request_id: params.request_id,
        ext_success: externalExecutionFilters.value.success,
        ext_page: params.page,
      },
    });
    const res = pluginCode
      ? await admin.pluginHookExecutions(pluginCode, params)
      : await admin.listPluginHookExecutions({ ...params, plugin_code: '' });
...
onMounted(async () => {
  const pluginOptionsPromise = loadPluginOptions();
  tab.value = normalizeMainTab(tab.value);
  if (tab.value === 'overview') {
    await Promise.all([refreshDeliveries(), refreshCircuits(), pluginOptionsPromise]);
    return;
  }
