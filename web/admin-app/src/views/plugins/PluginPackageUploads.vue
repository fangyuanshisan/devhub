  ...
  <section class="plugin-upload-page" data-testid="plugin-package-upload-lifecycle-page">
    <header class="page-head">
      ...
      <div class="head-actions">
        <el-button :loading="cleanupLoading" data-testid="upload-cleanup" @click="cleanupUploads">清理 blocked/failed/expired</el-button>
    ...
    <el-drawer v-model="drawer" size="720px" title="上传包详情" data-testid="upload-detail-drawer">
      <template v-if="detail">
        ...
        <div class="actions" data-testid="upload-actions">
          ...
          <el-button type="danger" :disabled="!actionEnabled('delete')" :loading="actionLoading === 'delete'" data-testid="upload-delete" @click="runAction('delete')">删除文件和记录</el-button>
        </div>
...
async function runAction(name) {
  ...
  try {
    ...
    if (name === 'delete') {
      await deletePluginPackageUpload(uploadId);
      detail.value = null;
      drawer.value = false;
    }
...
async function cleanupUploads() {
  ...
  errorText.value = '';
  try {
    const preview = await cleanupPluginPackageUploads({ dry_run: true, statuses: ['blocked', 'failed', 'expired'] });
    if (!preview.will_delete_count) {
      ElMessage.info('没有可清理的上传包');
      return;
    }
    await ElMessageBox.confirm(`将删除 ${preview.will_delete_count} 个上传包，预计释放 ${preview.will_free_bytes || 0} bytes。`, '清理确认', { type: 'warning' });
    const res = await cleanupPluginPackageUploads({ dry_run: false, statuses: ['blocked', 'failed', 'expired'], confirm_token: preview.confirm_token });
