  ...
  <section class="plugin-domain-page" data-testid="plugin-publishers-domain">
    <div class="plugin-domain-header">
      <div>
        ...
        <h2>安全与发布者</h2>
        <p class="muted">聚合可信发布者、签名信任、公钥标识、影响分析和配置密钥轮换，安全相关操作都从这里进入。</p>
      </div>
    </div>
...
function AdvancedTrustGuidance() {
  ...
  return h(ElCard, { shadow: 'never', class: 'domain-note-card' }, () => [
    ...
    h(ElText, { type: 'info' }, () => '公钥、key_id、可信级别和影响分析仍可访问，但不再默认占据安全主视图。'),
...
const tabLoading = () => h('div', { class: 'tab-state' }, [
  h(ElText, { type: 'info' }, () => '正在加载安全与发布者内容...'),
]);
...
const tabs = [
  { name: 'list', label: '可信发布者', component: lazyTab(() => import('./PluginTrustedPublishers.vue')) },
  { name: 'config-keys', label: '配置密钥', component: lazyTab(() => import('./PluginConfigKeys.vue')) },
...
const visibleTabs = computed(() => {
  const primary = new Set(['list', 'config-keys', 'advanced']);
