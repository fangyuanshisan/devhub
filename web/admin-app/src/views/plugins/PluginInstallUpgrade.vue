<template>
  <section class="plugin-page" data-testid="plugin-install-page">
    <AdminPageHeader
      title="安装升级"
      description="Manifest 校验、预检、安装与升级入口。不会执行第三方代码，不支持远程下载与动态加载。"
      :breadcrumbs="['安装与治理']"
      testid="plugin-install-page-header"
    >
      <template #actions>
    ...
    <el-alert
      ...
      :closable="false"
      class="mb"
      title="安装与升级分层：远程索引不等于自动安装，暂存区不等于已安装，预检通过不等于可启用，启用前检查通过后才允许进入启用流程，升级不会执行第三方代码。"
    />

    <el-card v-if="packageTab === 'template'" shadow="never" class="mb" data-testid="plugin-package-template-card">
      <template #header>
        <div class="card-head">
          <strong>创建插件模板</strong>
          <span class="muted">在 storage/plugins/drafts/{code} 下生成声明型安全模板；不安装、不启用、不执行 SQL。</span>
        </div>
      </template>
      ...
      <el-form :model="templateForm" label-width="120px" class="template-form" data-testid="plugin-package-template-form">
        <div class="template-grid">
          ...
          <el-form-item label="插件类型">
            <el-select v-model="templateForm.plugin_type" data-testid="plugin-template-type" placeholder="选择插件类型">
              <el-option v-for="item in pluginTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item v-if="isContentPluginTemplate" label="内容数据类型">
            <el-input v-model="templateForm.content_type" data-testid="plugin-template-content-type" placeholder="demo_notice_item" clearable @input="templateContentTypeTouched = true" />
            <div class="form-tip">用于系统内部识别内容数据，例如 friend_link、faq、announcement_block。</div>
          </el-form-item>
          <el-form-item v-if="isContentPluginTemplate" label="内容显示名称">
          ...
          <el-form-item label="发布者 / 作者">
            <el-select v-model="templateForm.publisher_mode" data-testid="plugin-template-author" placeholder="选择发布者" @change="applyTemplatePublisher">
              <el-option label="DevHub Team" value="devhub_team" />
              <el-option label="当前组织" value="current_org" />
              <el-option label="当前用户" value="current_user" />
              <el-option v-if="trustedPublisherOptions.length" label="可信发布者" value="trusted_publisher" />
              <el-option label="自定义" value="custom" />
            </el-select>
            <div v-if="!trustedPublisherOptions.length" class="form-tip">暂无可信发布者，可先使用 DevHub Team 或当前用户。</div>
          </el-form-item>
          <el-form-item v-if="templateForm.publisher_mode === 'trusted_publisher'" label="可信发布者">
            <el-select v-model="templateForm.publisher_id" placeholder="选择可信发布者" @change="applyTemplatePublisher">
              <el-option v-for="item in trustedPublisherOptions" :key="item.publisher_id || item.id" :label="item.name || item.publisher_id" :value="item.publisher_id || item.name" />
            </el-select>
          </el-form-item>
        </div>
          ...
          <el-collapse-item title="高级设置" name="advanced">
            <div class="template-grid">
              <el-form-item label="插件编码">
                <el-input v-model="templateForm.code" data-testid="plugin-template-code" placeholder="demo_notice" clearable @input="templateCodeTouched = true">
                  <template #append>
                    <el-button data-testid="plugin-template-code-regenerate" @click="regenerateTemplateCode">重新生成</el-button>
                  </template>
                </el-input>
                <div class="form-tip">插件编码已自动生成，可在高级设置中修改；仅允许小写字母、数字、下划线和中划线。</div>
              </el-form-item>
              <el-form-item v-if="templateForm.publisher_mode === 'custom'" label="自定义作者">
                <el-input v-model="templateForm.author" placeholder="发布者名称" clearable />
              </el-form-item>
              <el-form-item v-if="templateForm.plugin_type === 'external_service'" label="Hook 路径">
                <el-input v-model="templateForm.health_check_path" placeholder="/hooks/content.after_create" clearable />
              </el-form-item>
              <el-form-item v-if="templateForm.plugin_type === 'external_service'" label="超时 ms">
                <el-input-number v-model="templateForm.timeout_ms" :min="100" :max="10000" :step="100" />
              </el-form-item>
              <el-form-item v-if="templateForm.plugin_type === 'external_service'" label="失败策略">
                <el-select v-model="templateForm.failure_policy">
                  <el-option label="告警" value="warn" />
                  <el-option label="记录日志" value="log" />
                  <el-option label="忽略" value="ignore" />
                </el-select>
              </el-form-item>
              <el-form-item v-if="templateForm.plugin_type === 'frontend_mount'" label="挂载点">
                <el-select v-model="templateForm.mount_point">
                  <el-option label="后台插件详情预览" value="admin.plugin.detail.preview" />
                  <el-option label="前台首页官方区块" value="frontend.home.section" />
                  <el-option label="子站首页官方区块" value="frontend.community.section" />
                </el-select>
              </el-form-item>
              <el-form-item v-if="templateForm.plugin_type === 'frontend_mount'" label="组件 key">
                <el-select v-model="templateForm.component_key">
                  <el-option label="官方公告卡片" value="official.announcement.card" />
                </el-select>
              </el-form-item>
            </div>
          </el-collapse-item>
        ...
        <div class="filter-actions" style="justify-content: space-between">
          ...
          <div class="filter-actions">
            ...
            <el-button plain :loading="templateLoading" data-testid="plugin-template-export" @click="exportPackageTemplateZip">下载 zip</el-button>
            <el-button type="primary" :loading="templateLoading" data-testid="plugin-template-create" @click="createPackageTemplate">创建模板</el-button>
          </div>
        </div>
      </el-form>
      ...
      <el-card v-if="templateResult" shadow="never" class="mb" data-testid="plugin-template-result">
        ...
        <el-descriptions :column="2" border class="mb">
          ...
          <el-descriptions-item label="插件类型">{{ pluginTypeLabel(templateResult.template?.plugin_type) }}</el-descriptions-item>
          <el-descriptions-item label="插件编码">{{ templateResult.template?.code || '-' }}</el-descriptions-item>
          <el-descriptions-item v-if="templateResult.template?.content_type" label="内容数据类型">{{ templateResult.template?.content_type }}</el-descriptions-item>
          <el-descriptions-item v-if="templateResult.template?.content_name" label="内容显示名称">{{ templateResult.template?.content_name }}</el-descriptions-item>
          <el-descriptions-item label="发布者 / 作者">{{ templateResult.template?.author || '-' }}</el-descriptions-item>
        ...
        <div class="result-grid mb">
          <div class="result-box">
            <h4>文件树</h4>
            <div class="tag-wrap">
              <el-tag v-for="file in templateResult.template?.file_tree || templateResult.template?.files || []" :key="file" effect="plain">{{ file }}</el-tag>
            </div>
          </div>
          <div class="result-box">
            <h4>权限 / 菜单 / migrations</h4>
            <div class="tag-wrap">
              <el-tag v-for="item in templateResult.template?.permissions || []" :key="`p-${item}`" effect="plain">{{ item }}</el-tag>
              <el-tag v-for="item in templateResult.template?.menus || []" :key="`m-${item}`" type="success" effect="plain">{{ item }}</el-tag>
              <el-tag v-for="item in templateResult.template?.migrations || []" :key="`g-${item}`" type="warning" effect="plain">{{ item }}</el-tag>
            </div>
          </div>
          <div v-if="(templateResult.template?.hooks || []).length || (templateResult.template?.frontend_mounts || []).length || templateResult.template?.external_service" class="result-box">
            <h4>扩展声明</h4>
            <div class="tag-wrap">
              <el-tag v-for="item in templateResult.template?.hooks || []" :key="`h-${item}`" type="info" effect="plain">{{ item }}</el-tag>
              <el-tag v-for="item in templateResult.template?.frontend_mounts || []" :key="`${item.mount_point}-${item.component_key}`" type="info" effect="plain">{{ item.mount_point }} / {{ item.component_key }}</el-tag>
              <el-tag v-if="templateResult.template?.external_service" type="warning" effect="plain">external_service</el-tag>
            </div>
          </div>
          <div v-if="(templateResult.template?.conflicts || []).length" class="result-box">
            <h4>命名冲突</h4>
            <ul class="result-list">
              <li v-for="(item, idx) in templateResult.template.conflicts" :key="`tpl-c-${idx}`">{{ item.message }} {{ item.value ? `(${item.value})` : '' }} {{ item.suggestion || '' }}</li>
            </ul>
          </div>
          <div class="result-box">
            <h4>警告 / 错误</h4>
    ...
    <div class="filter-panel mb" data-testid="plugin-package-repo-panel">
      ...
      <div class="filter-actions">
        <el-radio-group v-model="repoScope" data-testid="plugin-package-repo-scope" @change="applyRepositoryScope">
          <el-radio-button label="packages">正式仓库包</el-radio-button>
          <el-radio-button label="drafts">草稿包</el-radio-button>
          <el-radio-button label="all">全部本地包</el-radio-button>
        </el-radio-group>
        <el-input v-model="repoRoot" data-testid="plugin-package-repo-root" placeholder="storage/plugins/packages / storage/plugins/drafts / __all__" clearable @input="repoScope = 'custom'" />
        ...
        <el-dropdown split-button type="danger" plain :loading="repoCleanupLoading" data-testid="plugin-package-repo-cleanup" @click="cleanupRepository('test_packages')">
          清理测试包
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="cleanupRepository('test_packages')">清理测试包</el-dropdown-item>
              <el-dropdown-item @click="cleanupRepository('uninstalled')">清理未安装包</el-dropdown-item>
              <el-dropdown-item @click="cleanupRepository('blocked_invalid')">清理 blocked / invalid</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </div>
    ...
    <el-alert v-if="repoCleanupPreview && !repoCleanupVisible" type="info" show-icon :closable="true" class="mb" @close="repoCleanupPreview = null">
      <template #title>
        上次清理预览：可清理 {{ repoCleanupPreview.will_delete_count || 0 }} 个，跳过 {{ repoCleanupPreview.skipped_count || 0 }} 个。
      </template>
    </el-alert>
      ...
      <el-table :data="repoItems" border size="small" v-loading="repoLoading" data-testid="plugin-package-repo-table">
        ...
        <el-table-column label="分区" width="120">
          <template #default="{ row }">
            <el-tag :type="repositoryScopeType(row.repository_scope)" effect="plain">{{ repositoryScopeLabel(row.repository_scope, row.path) }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="310" fixed="right">
          <template #default="{ row }">
            ...
            <span data-testid="plugin-package-repo-promote-btn" style="display: inline-flex">
              <el-tooltip :content="repoPromoteDisabledReason(row)" placement="top" :disabled="canPromoteRepoPackage(row)">
                <el-button link type="primary" :disabled="!canPromoteRepoPackage(row)" @click.stop="promoteRepoPackage(row)">转正</el-button>
              </el-tooltip>
            </span>
            <span v-if="isFormalRepoPackage(row)" data-testid="plugin-package-repo-install-btn" style="display: inline-flex">
              <el-tooltip :content="repoInstallDisabledReason(row)" placement="top" :disabled="canInstallRepoPackage(row)">
                <el-button
                  link
                  type="success"
                  :disabled="!canInstallRepoPackage(row)"
            ...
            <span data-testid="plugin-package-repo-delete-btn" style="display: inline-flex">
              <el-tooltip :content="row.delete_disabled_reason || '删除插件包目录'" placement="top" :disabled="row.can_delete">
                <el-button link type="danger" :disabled="!row.can_delete" @click.stop="deleteRepoPackage(row)">删除</el-button>
              </el-tooltip>
            </span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="repoCleanupVisible" width="860px" destroy-on-close data-testid="plugin-package-cleanup-dialog">
      <template #header>
        <strong>{{ cleanupScopeTitle(repoCleanupScope) }}</strong>
      </template>
      <el-alert
        type="warning"
        show-icon
        :closable="false"
        class="mb"
        title="清理只删除未安装本地仓库包的数据库记录和 storage 文件；已安装、已启用或有运行中任务的包会被后端强制跳过。"
      />
      <el-skeleton v-if="repoCleanupLoading && !repoCleanupPreview" :rows="6" animated />
      <template v-else-if="repoCleanupPreview">
        <el-empty v-if="!repoCleanupPreview.will_delete_count" description="没有可清理的测试包 / 未安装包" />
        <template v-else>
          <div class="mb" data-testid="plugin-package-cleanup-summary">
            <el-tag type="danger" effect="plain">将删除 {{ repoCleanupPreview.will_delete_count || 0 }} 个包</el-tag>
            <el-tag type="warning" effect="plain">文件 {{ repoCleanupPreview.will_delete_files || 0 }} 个</el-tag>
            <el-tag effect="plain">预计释放 {{ formatBytes(repoCleanupPreview.will_free_bytes || 0) }}</el-tag>
            <el-tag type="info" effect="plain">跳过 {{ repoCleanupPreview.skipped_count || 0 }} 个</el-tag>
          </div>
          <el-descriptions :column="2" border class="mb">
            <el-descriptions-item label="状态分布">{{ formatCountMap(repoCleanupPreview.status_counts) }}</el-descriptions-item>
            <el-descriptions-item label="跳过分布">{{ formatCountMap(repoCleanupPreview.skipped_counts) }}</el-descriptions-item>
          </el-descriptions>
          <el-table :data="repoCleanupPreview.items || []" size="small" border max-height="360" data-testid="plugin-package-cleanup-items">
            <el-table-column label="结果" width="110">
              <template #default="{ row }">
                <el-tag :type="row.can_delete ? 'danger' : 'info'" effect="plain">{{ row.can_delete ? '将清理' : '已跳过' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="plugin_code" label="plugin_code" min-width="170" />
            ...
            <el-table-column prop="reason" label="原因" min-width="220">
              <template #default="{ row }">{{ row.reason || pluginReasonText(row.blocked_code) || '-' }}</template>
            </el-table-column>
          </el-table>
        </template>
      </template>
      <template #footer>
        <el-button @click="repoCleanupVisible = false">取消</el-button>
        <el-button
          type="danger"
          :loading="repoCleanupLoading"
          :disabled="!repoCleanupPreview || !repoCleanupPreview.will_delete_count"
          data-testid="plugin-package-cleanup-confirm"
          @click="confirmRepositoryCleanup"
        >
          确认清理
        </el-button>
      </template>
    ...
    <el-dialog v-model="repoInstallVisible" width="820px" destroy-on-close data-testid="plugin-package-repo-install-dialog">
      ...
      <section v-if="repoInstallVisible" class="action-panel in-drawer" data-testid="plugin-package-repo-install-content">
        ...
        <div v-if="repoInstallDuplicatePluginCode" class="duplicate-actions mb" data-testid="plugin-package-duplicate-actions">
          <span class="muted">已安装插件应走配置或升级流程：</span>
          <el-button size="small" type="primary" plain @click="openInstalledPluginConfig(repoInstallDuplicatePluginCode)">配置插件</el-button>
          <el-button size="small" type="warning" plain @click="openInstalledPluginExternalService(repoInstallDuplicatePluginCode)">配置 external_service</el-button>
          <el-button size="small" plain @click="openPluginVersions(repoInstallDuplicatePluginCode)">查看版本 / 升级差异</el-button>
        </div>
      ...
      <section class="action-panel in-drawer" data-testid="plugin-manifest-panel">
        ...
        <div v-else data-testid="plugin-result-panel">
          ...
          <template v-if="manifestDialogAction === 'upgrade' || manifestDialogAction === 'upgrade-dry-run'">
            <div class="top-risk-stack" data-testid="plugin-upgrade-top-risk-items">
              <el-alert v-if="(resultUpgrade.blocking_items || []).length" title="阻断项：该升级不能继续，confirm 也不能绕过。" type="error" show-icon :closable="false" class="mb">
                <ul class="result-list">
                  <li v-for="(item, idx) in resultUpgrade.blocking_items || []" :key="`top-blocking-${idx}`">{{ riskItemText(item) }}</li>
                </ul>
              </el-alert>
              <el-alert v-if="(resultUpgrade.warnings || []).length" title="警告项：继续升级前必须显式确认。" type="warning" show-icon :closable="false" class="mb">
                <ul class="result-list">
                  <li v-for="(item, idx) in resultUpgrade.warnings || []" :key="`top-warning-${idx}`">{{ riskItemText(item) }}</li>
                </ul>
              </el-alert>
            </div>
            <el-descriptions :column="2" border class="mb" data-testid="plugin-result-summary">
              <el-descriptions-item label="插件编码">{{ resultUpgrade.plugin_code || manifestDialogTarget?.code || targetCode || '-' }}</el-descriptions-item>
              ...
              <el-descriptions-item label="回滚边界" :span="2">{{ resultUpgrade.rollback_boundary?.message || '当前不提供完整自动回滚' }}</el-descriptions-item>
              <el-descriptions-item label="受影响能力" :span="2">
                <div class="tag-wrap">
                  <el-tag :type="resultUpgrade.impact_summary?.migration_affected ? 'warning' : 'info'" effect="plain">migration {{ resultUpgrade.impact_summary?.migration_affected ? '涉及' : '未涉及' }}</el-tag>
                  <el-tag :type="resultUpgrade.impact_summary?.external_service_affected ? 'warning' : 'info'" effect="plain">external_service {{ resultUpgrade.impact_summary?.external_service_affected ? '涉及' : '未涉及' }}</el-tag>
                  <el-tag :type="resultUpgrade.impact_summary?.frontend_mount_affected ? 'warning' : 'info'" effect="plain">frontend_mount {{ resultUpgrade.impact_summary?.frontend_mount_affected ? '涉及' : '未涉及' }}</el-tag>
                  <el-tag :type="resultUpgrade.impact_summary?.menu_route_affected ? 'warning' : 'info'" effect="plain">menu / route {{ resultUpgrade.impact_summary?.menu_route_affected ? '涉及' : '未涉及' }}</el-tag>
                </div>
              </el-descriptions-item>
            </el-descriptions>
            <div class="result-grid mb">
              ...
              <div class="result-box full-width" data-testid="plugin-upgrade-diff-sections">
                <h4>结构化差异分区</h4>
                <el-tabs v-if="(resultUpgrade.diff_sections || []).length" type="border-card" class="upgrade-tabs">
                  <el-tab-pane v-for="section in resultUpgrade.diff_sections || []" :key="section.section" :label="`${section.title || section.section} (${(section.items || []).length})`" :name="section.section">
                    <el-table :data="section.items || []" border stripe :empty-text="t('common.none')">
                      <el-table-column label="风险" width="110">
                        <template #default="{ row }"><el-tag :type="diffRiskType(row.risk_level)" effect="plain">{{ diffRiskLabel(row.risk_level) }}</el-tag></template>
                      </el-table-column>
                      ...
                      <el-table-column label="升级前" min-width="220">
                        <template #default="{ row }"><pre class="inline-code">{{ formatDisplayValue(row.before) }}</pre></template>
                      </el-table-column>
                      <el-table-column label="升级后" min-width="220">
                        <template #default="{ row }"><pre class="inline-code">{{ formatDisplayValue(row.after) }}</pre></template>
                      </el-table-column>
                    </el-table>
                  </el-tab-pane>
                </el-tabs>
                <span v-else class="muted">未发现结构化差异。</span>
              </div>
              <div class="result-box full-width" data-testid="plugin-upgrade-section-plans">
                <h4>分区升级计划</h4>
                <el-alert title="dry-run 只生成计划，不执行 SQL，不执行 migration down，也不执行第三方代码。" type="info" show-icon :closable="false" class="mb" />
                <div class="section-plan-grid">
                  <div v-for="plan in upgradeSectionPlans(resultUpgrade)" :key="plan.key" class="section-plan-card">
                    <div class="plan-title">{{ plan.title }}</div>
                    <div class="muted">{{ plan.message || '未发现该分区变更。' }}</div>
                    <div class="tag-wrap mt8">
                      <el-tag effect="plain">新增 {{ plan.added ?? 0 }}</el-tag>
                      <el-tag effect="plain">删除 {{ plan.removed ?? 0 }}</el-tag>
                      <el-tag effect="plain">变更 {{ plan.changed ?? 0 }}</el-tag>
                      <el-tag :type="(plan.blocked ?? 0) > 0 ? 'danger' : (plan.high_risk ?? 0) > 0 ? 'warning' : 'info'" effect="plain">高风险 {{ plan.high_risk ?? 0 }} / 阻断 {{ plan.blocked ?? 0 }}</el-tag>
                    </div>
                  </div>
                </div>
              </div>
              <div class="result-box full-width" data-testid="plugin-upgrade-frontend-mount-plan">
                <h4>前端挂载差异</h4>
                <el-table :data="frontendMountDiffRows(resultUpgrade)" border stripe :empty-text="t('common.none')">
                  <el-table-column prop="type" label="类型" width="100">
                    <template #default="{ row }">{{ diffTypeLabel(row.type) }}</template>
                  </el-table-column>
                  <el-table-column prop="mount_point" label="mount_point" min-width="190" />
                  <el-table-column prop="component_key" label="component_key" min-width="190" />
                  <el-table-column prop="render_mode" label="render_mode" min-width="150" />
                  <el-table-column label="allowlist" width="120">
                    <template #default="{ row }"><el-tag :type="row.blocked ? 'danger' : 'success'" effect="plain">{{ row.blocked ? '阻断' : '通过' }}</el-tag></template>
                  </el-table-column>
              ...
              <div class="result-box full-width" data-testid="plugin-upgrade-risk-items">
                ...
                <div v-if="resultUpgrade.failure_stage || resultUpgrade.failure_reason || resultUpgrade.request_id || resultUpgrade.next_step" class="mb">
                  <div v-if="resultUpgrade.failure_reason" class="muted">失败原因：{{ resultUpgrade.failure_reason }}</div>
                  <div v-if="resultUpgrade.request_id" class="muted">request_id：{{ resultUpgrade.request_id }}</div>
                  <div v-if="resultUpgrade.next_step" class="muted">下一步建议：{{ resultUpgrade.next_step }}</div>
                  <div class="muted">处理提示：先查看 admin_logs；如涉及 Hook / external_service，再查看 hook_executions。blocked 不可通过 confirm 继续。</div>
                </div>
                <el-alert v-if="(resultUpgrade.blocking_items || []).length" title="阻断项" type="error" show-icon :closable="false" class="mb">
                  <ul class="result-list">
                    <li v-for="(item, idx) in resultUpgrade.blocking_items || []" :key="`blocking-${idx}`">{{ riskItemText(item) }}</li>
                  </ul>
                </el-alert>
                <el-alert v-if="(resultUpgrade.warnings || []).length" title="警告项" type="warning" show-icon :closable="false" class="mb">
                  <ul class="result-list">
                    <li v-for="(item, idx) in resultUpgrade.warnings || []" :key="`warn-${idx}`">{{ riskItemText(item) }}</li>
                  </ul>
                </el-alert>
              ...
              <div class="result-box full-width" data-testid="plugin-upgrade-dependency-diff">
                ...
                <div class="tag-wrap mb">
                  <el-tag effect="plain">新增 {{ (resultUpgrade.dependency_diff?.added || []).length }}</el-tag>
                  <el-tag effect="plain">删除 {{ (resultUpgrade.dependency_diff?.removed || []).length }}</el-tag>
                  <el-tag effect="plain">版本变化 {{ (resultUpgrade.dependency_diff?.version_changed || []).length }}</el-tag>
                  <el-tag effect="plain">required 变化 {{ (resultUpgrade.dependency_diff?.required_changed || []).length }}</el-tag>
                </div>
                <el-table :data="dependencyDiffRows(resultUpgrade.dependency_diff)" border stripe :empty-text="t('common.none')">
                  <el-table-column prop="change" label="变化" width="130" />
                  <el-table-column prop="code" label="依赖插件" min-width="160" />
                  <el-table-column prop="version" label="版本约束" min-width="160" />
                  <el-table-column label="required" width="110">
                    <template #default="{ row }">{{ row.required ? 'required' : 'optional' }}</template>
                  </el-table-column>
...
import {
  ...
  promotePluginPackageRepository,
  previewPluginPackageTemplate,
  previewPluginPackageCleanup,
  plugins,
  upgradePlugin,
...
const templateForm = ref({
  ...
  publisher_id: '',
  mount_point: 'admin.plugin.detail.preview',
  component_key: 'official.announcement.card',
  health_check_path: '/hooks/content.after_create',
  timeout_ms: 3000,
  failure_policy: 'warn',
  with_config: true,
  with_hooks: true,
...
const pluginTypeOptions = [
  { label: '内容型插件', value: 'content' },
  { label: '外部服务型插件', value: 'external_service' },
  { label: '后台工具型插件', value: 'admin_tool' },
  { label: '前端挂载型插件', value: 'frontend_mount' },
];
const templateCodeTouched = ref(false);
const templateContentTypeTouched = ref(false);
const templatePluginTypeReverting = ref(false);
const trustedPublisherOptions = ref([]);
const currentAdminName = ref('');
const currentOrgName = ref('当前组织');
const isContentPluginTemplate = computed(() => templateForm.value.plugin_type === 'content');
...
const repositoryScopeRoots = {
  packages: 'storage/plugins/packages',
  drafts: 'storage/plugins/drafts',
  all: '__all__',
};
const repoScope = ref('packages');
const repoRoot = ref('storage/plugins/packages');
const repoLoading = ref(false);
const repoCleanupLoading = ref(false);
const repoError = ref('');
const repoItems = ref([]);
...
const repoCleanupVisible = ref(false);
const repoCleanupPreview = ref(null);
const repoCleanupScope = ref('test_packages');
const repoCleanupConfirmToken = ref('');
...
const repoInstallDuplicatePluginCode = ref('');
...
watch(() => templateForm.value.name, (name) => {
  if (!templateCodeTouched.value) {
    templateForm.value.code = slugifyPluginCode(name);
  }
  if (!templateContentTypeTouched.value && templateForm.value.plugin_type === 'content') {
    templateForm.value.content_type = defaultContentType(templateForm.value.code || slugifyPluginCode(name));
  }
  if (!String(templateForm.value.content_name || '').trim()) {
    templateForm.value.content_name = String(name || '').trim();
  }
});

watch(() => templateForm.value.code, (code) => {
  if (!templateContentTypeTouched.value && templateForm.value.plugin_type === 'content') {
    templateForm.value.content_type = defaultContentType(code);
  }
});

watch(() => templateForm.value.plugin_type, async (next, prev) => {
  if (!prev || next === prev) return;
  if (templatePluginTypeReverting.value) {
    templatePluginTypeReverting.value = false;
    return;
  }
  const ok = await ElMessageBox.confirm('切换插件类型会重置不相关字段，是否继续？', '切换插件类型', { type: 'warning', confirmButtonText: '继续', cancelButtonText: '取消' }).then(() => true).catch(() => false);
  if (!ok) {
    templatePluginTypeReverting.value = true;
    templateForm.value.plugin_type = prev;
    return;
  }
  if (next !== 'content') {
    templateForm.value.content_type = '';
    templateForm.value.content_name = '';
    templateContentTypeTouched.value = false;
  } else {
    templateForm.value.content_type = defaultContentType(templateForm.value.code || slugifyPluginCode(templateForm.value.name));
    templateForm.value.content_name = String(templateForm.value.name || '').trim();
  }
  templateForm.value.with_hooks = next === 'content' || next === 'external_service';
  templateForm.value.with_migration = next === 'content';
});

onMounted(load);

function templatePayload() {
  const isContent = templateForm.value.plugin_type === 'content';
  return {
    code: String(templateForm.value.code || '').trim(),
    name: String(templateForm.value.name || '').trim(),
    plugin_type: String(templateForm.value.plugin_type || 'content').trim(),
    content_type: isContent ? String(templateForm.value.content_type || '').trim() : '',
    content_name: isContent ? String(templateForm.value.content_name || '').trim() : '',
    description: String(templateForm.value.description || '').trim(),
    author: String(templateForm.value.author || '').trim(),
    publisher_mode: String(templateForm.value.publisher_mode || '').trim(),
    publisher_id: String(templateForm.value.publisher_id || '').trim(),
    mount_point: templateForm.value.plugin_type === 'frontend_mount' ? String(templateForm.value.mount_point || '').trim() : '',
    component_key: templateForm.value.plugin_type === 'frontend_mount' ? String(templateForm.value.component_key || '').trim() : '',
    health_check_path: templateForm.value.plugin_type === 'external_service' ? String(templateForm.value.health_check_path || '').trim() : '',
    timeout_ms: templateForm.value.plugin_type === 'external_service' ? Number(templateForm.value.timeout_ms || 0) : 0,
    failure_policy: templateForm.value.plugin_type === 'external_service' ? String(templateForm.value.failure_policy || '').trim() : '',
...
function slugifyPluginCode(value) {
  const base = String(value || '').trim().toLowerCase();
  let out = base.replace(/[^a-z0-9_-]+/g, '_').replace(/^[_-]+|[_-]+$/g, '').replace(/_{2,}/g, '_');
  if (!/^[a-z]/.test(out)) out = base ? `plugin_${hashText(base).slice(0, 8)}` : '';
...
function defaultContentType(code) {
  const value = String(code || '').replace(/[-]+/g, '_').replace(/^_+|_+$/g, '');
  if (!value) return '';
  return value.endsWith('_item') ? value : `${value.replace(/s$/, '')}_item`;
}

function regenerateTemplateCode() {
  templateCodeTouched.value = false;
  templateForm.value.code = slugifyPluginCode(templateForm.value.name);
  if (templateForm.value.plugin_type === 'content' && !templateContentTypeTouched.value) {
    templateForm.value.content_type = defaultContentType(templateForm.value.code);
  }
}

function pluginTypeLabel(value) {
  return pluginTypeOptions.find((item) => item.value === value)?.label || value || '-';
}

function applyTemplatePublisher() {
  const mode = String(templateForm.value.publisher_mode || '');
  if (mode === 'devhub_team') templateForm.value.author = 'DevHub Team';
  if (mode === 'current_org') templateForm.value.author = currentOrgName.value || '当前组织';
  if (mode === 'current_user') templateForm.value.author = currentAdminName.value || '当前用户';
  if (mode === 'trusted_publisher') {
    const item = trustedPublisherOptions.value.find((it) => String(it.publisher_id || it.name) === String(templateForm.value.publisher_id));
    templateForm.value.author = item?.name || item?.publisher_id || '';
  }
}
...
function formatAPIError(e, fallback) {
  ...
  diagnostics.forEach((item) => {
    if (item?.message) parts.push(`诊断：${item.message}`);
    if (item?.suggestion) parts.push(`处理：${item.suggestion}`);
  });
  return parts.join(' ') || String(e?.message || fallback);
}

function repositoryRootForPackagePath(path) {
...
function syncRepositoryRootForPackagePath(path) {
  const root = repositoryRootForPackagePath(path);
  if (root) {
    repoRoot.value = root;
    repoScope.value = repositoryScopeForRoot(root);
  }
}

function repositoryScopeForRoot(root) {
  const clean = String(root || '').trim().replace(/\\/g, '/').replace(/\/+$/g, '');
  if (clean === repositoryScopeRoots.packages) return 'packages';
  if (clean === repositoryScopeRoots.drafts) return 'drafts';
  if (clean === repositoryScopeRoots.all) return 'all';
  return 'custom';
}

function applyRepositoryScope(scope) {
  const nextRoot = repositoryScopeRoots[String(scope || '')];
  if (nextRoot) {
    repoRoot.value = nextRoot;
    scanRepository(true);
  }
}

function repositoryScopeForPath(path) {
  const clean = String(path || '').trim().replace(/\\/g, '/').replace(/\/+$/g, '');
  if (clean.startsWith(`${repositoryScopeRoots.packages}/`)) return 'packages';
  if (clean.startsWith(`${repositoryScopeRoots.drafts}/`)) return 'drafts';
  if (clean.startsWith('plugins-local/')) return 'plugins_local';
  if (clean.startsWith('examples/plugins/')) return 'examples';
  if (clean.startsWith('storage/plugins/exports/')) return 'exports';
  if (clean.startsWith('storage/plugins/staging/')) return 'staging';
  if (clean.startsWith('storage/plugins/quarantine/')) return 'quarantine';
  if (clean.startsWith('.devhub/plugins/')) return 'devhub';
  return 'custom';
}

function repositoryScopeLabel(scope, path = '') {
  const value = String(scope || repositoryScopeForPath(path) || '').trim();
  return {
    packages: '正式仓库',
    drafts: '草稿',
    plugins_local: '本地 fixture',
    examples: '示例',
    exports: '导出',
    staging: '暂存',
    quarantine: '隔离',
...
function repositoryScopeType(scope) {
  return {
    packages: 'success',
    drafts: 'warning',
    plugins_local: 'info',
...
async function promoteUploadedPackage() {
  ...
  try {
    ...
    if (res?.package_path) {
      ...
      syncRepositoryRootForPackagePath(res.package_path);
    }
    packageTab.value = 'repository';
...
async function createPackageTemplate() {
  ...
  try {
    ...
    if (path) {
      packagePath.value = path;
      syncRepositoryRootForPackagePath(path);
    }
    packageTab.value = 'repository';
    await scanRepository(true);
  } catch (e) {
...
async function exportPackageTemplateZip() {
  templateError.value = '';
  templateLoading.value = true;
  try {
    const blob = await exportPluginPackageTemplate(templatePayload());
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${templateForm.value.code || 'plugin-template'}.zip`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    ElMessage.success('模板 zip 已生成');
  } catch (e) {
    templateError.value = formatAPIError(e, '导出失败');
  } finally {
    templateLoading.value = false;
  }
}
...
async function load() {
  loading.value = true;
  try {
    const [list, userInfo, publishers] = await Promise.all([
      plugins(),
      me().catch(() => null),
      listTrustedPublishers({ status: 'trusted', page: 1, page_size: 50 }).catch(() => null),
    ]);
    items.value = list.items || [];
    const user = userInfo?.user || {};
    currentAdminName.value = user.nickname || user.username || '';
    currentOrgName.value = userInfo?.admin_context?.organization_name || userInfo?.admin_context?.site_name || '当前组织';
    trustedPublisherOptions.value = publishers?.items || [];
    applyTemplatePublisher();
  } finally {
    loading.value = false;
  }
}
...
async function scanRepository(resetPage = false) {
  ...
  repoScope.value = repositoryScopeForRoot(repoRoot.value);
  repoError.value = '';
...
async function deleteRepoPackage(row) {
  const path = String(row?.path || '').trim();
  if (!path) return;
  try {
    await ElMessageBox.confirm(row?.is_installed ? '已安装包不能直接删除，请先归档 / 软卸载插件。' : `确认删除本地插件仓库包 ${path}？`, '删除本地包', { type: 'warning' });
    repoLoading.value = true;
    await deletePluginPackageRepository({ path });
    ElMessage.success('本地仓库包已删除');
    await scanRepository();
  } catch (e) {
    if (e === 'cancel' || e === 'close') return;
    repoError.value = formatAPIError(e, '删除失败');
  } finally {
    repoLoading.value = false;
  }
}

function cleanupScopePayload(scope) {
  const selected = String(scope || 'test_packages');
  if (selected === 'uninstalled') {
    return { scope: 'uninstalled', include_blocked: true, include_invalid: true, include_promoted_uninstalled: true, include_warning_uninstalled: true };
  }
  if (selected === 'blocked_invalid') {
    return { scope: 'blocked_invalid', include_blocked: true, include_invalid: true, statuses: ['blocked', 'invalid'] };
  }
  return {
    scope: 'test_packages',
...
function cleanupScopeTitle(scope) {
  return {
    test_packages: '清理测试包',
    uninstalled: '清理未安装本地包',
    blocked_invalid: '清理 blocked / invalid 包',
  }[scope] || '批量清理';
}
...
function formatCountMap(map) {
  ...
  return entries.map(([key, value]) => `${genericStatusLabel(key)} ${value}`).join(' / ');
}
...
async function cleanupRepository(scope = 'test_packages') {
  repoCleanupLoading.value = true;
  repoError.value = '';
  repoCleanupScope.value = scope;
  repoCleanupPreview.value = null;
  repoCleanupConfirmToken.value = '';
  repoCleanupVisible.value = true;
  try {
    const preview = await previewPluginPackageCleanup({ ...cleanupScopePayload(scope), dry_run: true });
    repoCleanupPreview.value = preview;
    repoCleanupConfirmToken.value = preview.confirm_token || '';
    if (!preview.will_delete_count) {
      ElMessage.info('没有可清理的测试包 / 未安装包');
      return;
    }
  } catch (e) {
    repoError.value = formatAPIError(e, '清理失败');
  } finally {
    repoCleanupLoading.value = false;
  }
}

async function confirmRepositoryCleanup() {
  if (!repoCleanupPreview.value?.will_delete_count) return;
  repoCleanupLoading.value = true;
  repoError.value = '';
  try {
    const res = await cleanupPluginPackageRepository({
      ...cleanupScopePayload(repoCleanupScope.value),
      dry_run: false,
      confirm_token: repoCleanupConfirmToken.value,
    });
    repoCleanupPreview.value = res;
    if (res.failed_count || (res.errors || []).length) {
      ElMessage.warning(`清理部分完成：已清理 ${res.deleted_count || 0}，失败 ${res.failed_count || (res.errors || []).length}`);
    } else {
      ElMessage.success(`清理完成：${res.deleted_count || 0}`);
      repoCleanupVisible.value = false;
    }
    await scanRepository(true);
  } catch (e) {
    repoError.value = formatAPIError(e, '清理失败');
...
function canInstallRepoPackage(row) {
  ...
  if (!path.startsWith(`${repositoryScopeRoots.packages}/`)) return false;
  if (row?.is_installed) return false;
...
function isFormalRepoPackage(row) {
  const path = String(row?.path || '').trim().replace(/\\/g, '/');
  return path.startsWith(`${repositoryScopeRoots.packages}/`);
}

function repoInstallDisabledReason(row) {
  const path = String(row?.path || '').trim().replace(/\\/g, '/');
  const scope = String(row?.repository_scope || repositoryScopeForPath(path));
  if (!path.startsWith(`${repositoryScopeRoots.packages}/`)) {
    if (scope === 'drafts') return '草稿包只能预检；请先转入 storage/plugins/packages 后再安装。';
    return '只有 storage/plugins/packages 下的正式仓库包允许安装。';
  }
  if (row?.is_installed) return '该插件已安装，无需重复安装。请打开插件详情配置插件或 external_service；更新插件包请进入版本仓库查看升级差异、提交审批并按审批流程执行。';
  if (!row?.manifest_found) return '缺少 manifest.json，不能安装。';
  if (!row?.code) return '未识别到插件编码，不能安装。';
  const status = String(row?.status || '').toLowerCase();
  if (status !== 'ok' && status !== 'warning') return '当前包预检未通过，不能安装。';
  if (String(row?.risk_level || '').toLowerCase() === 'blocked') return '风险校验已阻断，不能安装。';
  if (String(row?.checksum_status || '').toLowerCase() === 'failed') return 'checksum 校验失败，不能安装。';
  return '当前包不能安装。';
}

function canPromoteRepoPackage(row) {
  const path = String(row?.path || '').trim().replace(/\\/g, '/');
  const status = String(row?.status || 'ok').toLowerCase();
  const risk = String(row?.risk_level || '').toLowerCase();
  const checksum = String(row?.checksum_status || 'ok').toLowerCase();
  if (!path.startsWith(`${repositoryScopeRoots.drafts}/`)) return false;
  if (!row?.manifest_found) return false;
  if (!row?.code) return false;
  if (status !== 'ok' && status !== 'warning') return false;
  if (risk === 'blocked') return false;
...
function repoPromoteDisabledReason(row) {
  const path = String(row?.path || '').trim().replace(/\\/g, '/');
  if (!path.startsWith(`${repositoryScopeRoots.drafts}/`)) return '只有 storage/plugins/drafts 下的草稿包可以转入正式仓库。';
  if (!row?.manifest_found) return '缺少 manifest.json，不能转入正式仓库。';
  if (!row?.code) return '未识别到插件编码，不能转入正式仓库。';
  const status = String(row?.status || '').toLowerCase();
  if (status !== 'ok' && status !== 'warning') return '草稿包预检未通过，不能转入正式仓库。';
  if (String(row?.risk_level || '').toLowerCase() === 'blocked') return '风险校验已阻断，不能转入正式仓库。';
  if (String(row?.checksum_status || '').toLowerCase() === 'failed') return 'checksum 校验失败，不能转入正式仓库。';
  return '当前草稿包不能转入正式仓库。';
}

async function promoteRepoPackage(row) {
  const path = String(row?.path || '').trim();
  if (!path) return;
  try {
    await ElMessageBox.confirm(`确认将草稿包 ${path} 转入正式仓库？`, '转入正式仓库', { type: 'warning' });
    repoLoading.value = true;
    repoError.value = '';
    const res = await promotePluginPackageRepository({ path, force: false });
    ElMessage.success(res?.message || '已转入正式仓库');
    if (res?.package_path) {
      packagePath.value = res.package_path;
      syncRepositoryRootForPackagePath(res.package_path);
    }
    ...
    if (e === 'cancel' || e === 'close') return;
    repoError.value = formatAPIError(e, '转入正式仓库失败');
  } finally {
    repoLoading.value = false;
  }
}
...
async function openRepoInstall(row) {
  const path = String(row?.path || '').trim();
  if (!path) return;
  if (!canInstallRepoPackage(row)) {
    repoError.value = repoInstallDisabledReason(row);
    return;
  }
  repoInstallVisible.value = true;
  repoInstallLoading.value = true;
  repoInstallError.value = '';
  repoInstallDuplicatePluginCode.value = '';
  ...
  try {
    ...
    setRepoInstallError(e, '加载安装信息失败');
  } finally {
...
async function submitRepoInstallApproval() {
  ...
  try {
    ...
    setRepoInstallError(e, '提交审批失败');
  } finally {
    repoInstallLoading.value = false;
  }
}

function setRepoInstallError(e, fallback) {
  const data = e?.response?.data;
  const code = String(data?.code || '').trim();
  const message = String(data?.message || data?.error || '').trim();
  const suggestion = String(data?.suggestion || data?.details?.suggestion || '').trim();
  ...
  repoInstallError.value = parts.join(' ') || String(e?.message || fallback);
  repoInstallDuplicatePluginCode.value = code === 'plugin_package_already_installed'
    ? String(data?.details?.plugin_code || repoInstallDetail.value?.package?.code || '').trim()
    : '';
}

function openInstalledPluginConfig(code) {
  router.push({ path: '/plugins/overview', query: { tab: 'list', plugin_code: code, detail_tab: 'config' } });
}

function openInstalledPluginExternalService(code) {
  router.push({ path: '/plugins/overview', query: { tab: 'list', plugin_code: code, detail_tab: 'runtime' } });
}

function openPluginVersions(code) {
  router.push({ path: '/plugins/packages', query: { tab: 'versions', keyword: code } });
}
...
function resultCompatibility(result) {
  return result?.compatibility || result?.validation?.compatibility || {
    core_version: result?.current_core_version || '',
    compatible_core_version: result?.compatible_core_version || '',
...
function diffRiskLabel(level) {
  const risk = String(level || '').toLowerCase();
  if (risk === 'blocked') return '阻断';
...
function riskItemText(item) {
  if (!item) return '-';
  const parts = [item.message || item.code || '-'];
...
function upgradeSectionPlans(result) {
  return [
    { key: 'migration_plan', title: 'Migration 计划', ...(result?.migration_plan || {}) },
    { key: 'config_plan', title: '配置计划', ...(result?.config_plan || {}) },
    { key: 'permission_plan', title: '权限计划', ...(result?.permission_plan || {}) },
    { key: 'content_type_plan', title: '内容类型计划', ...(result?.content_type_plan || {}) },
    { key: 'hook_plan', title: 'Hook / external_service 计划', ...(result?.hook_plan || {}) },
    { key: 'frontend_mount_plan', title: '前端挂载计划', ...(result?.frontend_mount_plan || {}) },
  ];
}

function dependencyDiffRows(diff = {}) {
  const rows = [];
  const pushRows = (label, items, reason) => {
  ...
  pushRows('新增', diff.added, '新增依赖');
  pushRows('删除', diff.removed, '删除依赖');
  pushRows('版本变化', diff.version_changed, '依赖版本约束变化');
  pushRows('required 变化', diff.required_changed, 'required / optional 变化');
  return rows;
}

function frontendMountDiffRows(result) {
  const items = result?.frontend_mount_plan?.items || [];
  return items
    .filter((item) => String(item.section || '').includes('frontend_mounts') || String(item.path || '').includes('frontend_mounts'))
    .map((item) => {
      const mount = item.after && typeof item.after === 'object' ? item.after : item.before && typeof item.before === 'object' ? item.before : {};
      const message = item.message || '';
      return {
        type: item.type || '-',
        mount_point: mount.mount_point || item.path || '-',
        component_key: mount.component_key || '-',
        render_mode: mount.render_mode || 'official_component',
        blocked: String(item.risk_level || '').toLowerCase() === 'blocked' || /NOT_ALLOWED|不在官方允许列表|不支持/.test(message),
        message: message || '官方 allowlist 前端挂载差异',
      };
    });
}
</script>
...
.upgrade-tabs :deep(.el-tabs__content) {
  overflow-x: auto;
}
