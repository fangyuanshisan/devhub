  ...
  <section class="plugin-domain-page" data-testid="plugin-overview-domain">
    <div class="plugin-domain-header">
      <div>
        ...
        <h2>插件列表</h2>
        <p class="muted">日常状态、筛选、启停、配置和详情统一在插件列表处理；安装升级去安装与升级，投递与密钥去 Webhook 与回调，排障和追踪去运行与审计。</p>
      </div>
    </div>
...
function AdvancedGovernance() {
  ...
  return h(ElCard, { shadow: 'never', class: 'domain-note-card' }, () => [
    ...
    h('p', { class: 'muted' }, '低频能力仍可访问，但不再默认铺在插件列表首页。需要改配置、看挂载、查权限或排障时，再进入对应功能区。'),
    ...
    h(ElDescriptions, { column: 1, border: true, class: 'note-descriptions' }, () => [
      h(ElDescriptionsItem, { label: '日常入口' }, () => '优先使用插件列表处理启停、配置、异常和详情。'),
      h(ElDescriptionsItem, { label: '变更 / 排障' }, () => '安装升级去安装与升级，投递与密钥去 Webhook 与回调，操作历史和审计去运行与审计。'),
