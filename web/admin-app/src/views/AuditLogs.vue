<template>
  ...
  <el-card shadow="never">
    ...
    <el-table :data="rows" stripe border>
      ...
      <el-table-column label="操作人" width="150"><template #default="{ row }">{{ row.actor || '-' }}<div class="content-meta">UID {{ row.actor_user_id || '-' }} / {{ row.role || 'operator' }}</div></template></el-table-column>
...
const query = reactive({ site: 'portal', type: 'all', target_type: '', actor: '', action: '', target: '', page: 1, page_size: 20 });
const rows = ref([]);
const total = ref(0);
...
function reset() {
  Object.assign(query, { site: 'portal', type: 'all', target_type: '', actor: '', action: '', target: '', page: 1, page_size: 20 });
  load();
}
load();
</script>
