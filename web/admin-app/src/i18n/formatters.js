...
export function maskRef(value) {
  ...
  // Keep prefix hint (like whsec_/ctok_/tok_) but avoid exposing the whole reference in tables.
  const keepHead = Math.min(6, Math.max(2, s.indexOf('_') >= 0 ? s.indexOf('_') + 1 : 4));
