...
const withTab = (path, tab) => (to) => ({ path, query: { ...to.query, tab }, hash: to.hash });
const withPackageInstallTab = (to) => {
  const query = { ...to.query, tab: 'install' };
  return { path: '/plugins/packages', query, hash: to.hash };
};
...
export const menuRoutes = [
  ...
  { path: '/official-links', name: 'officialLinksPlugin', component: () => import('@/views/PluginContent.vue'), meta: { title: '友情链接管理', short: '友情链接', icon: 'Link', permission: 'official_links.link.manage', pluginCode: 'official_links', contentType: 'friend_link', hiddenInMenu: true, keepAlive: true, moduleKey: 'plugins', navGroupKey: 'manage', navPageKey: 'content' } },
  { path: '/feishu-links', name: 'feishuLinksPlugin', component: () => import('@/views/PluginContent.vue'), meta: { title: '飞书链接管理', short: '飞书链接', icon: 'Link', permission: 'plugin_a7b0cc04.link.audit', pluginCode: 'plugin_a7b0cc04', contentType: 'feishu_link', hiddenInMenu: true, keepAlive: true, moduleKey: 'plugins', navGroupKey: 'manage', navPageKey: 'content' } },
  ...
  { path: '/system/settings', redirect: withTab('/system', 'overview') },
  { path: '/system/effective-config', redirect: withTab('/system', 'effective') },
  { path: '/system/secret-center', redirect: withTab('/system', 'secret-center') },
  { path: '/system/secrets', redirect: withTab('/system', 'secret-center') },
  { path: '/system/external-service/http-allowlist', redirect: withTab('/system', 'external') },
  { path: '/system/http-allowlist', redirect: withTab('/system', 'external') },
];

export const pluginRoutes = [
  { path: '/plugins/overview', name: 'pluginsOverview', component: () => import('@/views/plugins/PluginOverviewDomain.vue'), meta: { title: '插件列表', permission: 'plugin.read', subNavGroup: 'plugins', subNavKey: 'overview', moduleKey: 'plugins', navGroupKey: 'governance', navPageKey: 'overview' } },
  { path: '/plugins/packages', name: 'pluginsPackages', component: () => import('@/views/plugins/PluginPackagesDomain.vue'), meta: { title: '安装与升级', permission: 'plugin.read', subNavGroup: 'plugins', subNavKey: 'packages', moduleKey: 'plugins', navGroupKey: 'governance', navPageKey: 'packages' } },
  { path: '/plugins/webhooks', name: 'pluginsWebhooks', component: () => import('@/views/plugins/PluginWebhooks.vue'), meta: { title: 'Webhook 与回调', permission: 'plugin.read', subNavGroup: 'plugins', subNavKey: 'webhooks', moduleKey: 'plugins', navGroupKey: 'governance', navPageKey: 'webhooks' } },
  { path: '/plugins/publishers', name: 'pluginsPublishers', component: () => import('@/views/plugins/PluginPublishersDomain.vue'), meta: { title: '安全与发布者', permission: 'plugin.read', subNavGroup: 'plugins', subNavKey: 'publishers', moduleKey: 'plugins', navGroupKey: 'governance', navPageKey: 'publishers' } },
  { path: '/plugins/runtime', name: 'pluginsRuntime', component: () => import('@/views/plugins/PluginRuntimeDomain.vue'), meta: { title: '运行与审计', permission: 'plugin.read', subNavGroup: 'plugins', subNavKey: 'runtime', moduleKey: 'plugins', navGroupKey: 'governance', navPageKey: 'runtime' } },
