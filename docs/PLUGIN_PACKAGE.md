...
v1.9.0-S1 补充：v1.9.0 是官方插件生态稳定版，不是插件包能力扩张版。本轮只用 fresh receiver 复核 `feishu_link` external_service Webhook 全链路；没有改变 upload / precheck / promote / install dry-run / install / upgrade dry-run / cleanup 的包治理语义，也没有开放插件市场、远程在线安装、自动安装、自动启用、package scripts 或第三方代码执行。

v1.9.0-S2 补充：SecretCenter 操作闭环只增强运维元数据、审计筛选和 effective-config 排障入口，不改变插件包格式或安装语义。插件包仍不得携带真实 token / Webhook Secret / Callback Token / Authorization；运行时敏感值通过来源配置写入 SecretCenter，并只以 `secret_ref/token_ref`、`usage_type/source_type/source_id/source_code` 等脱敏字段展示。

v1.9.0 发布归档补充：DevHub v1.9.0 官方插件生态稳定版已完成发布归档与发布后 targeted smoke，P0=0，P1=0，P2=0，版本可以冻结。建议先提交当前文档归档，再创建 v1.9.0 annotated tag。v1.9.0 冻结后不再追加新插件包能力；`v1.9.1` 仅作为维护候选，`v1.10.0` 作为更大治理和架构演进的规划阶段。
...
- v1.6.0-P0-02：上传包升级为可追踪生命周期对象，支持列表、详情、重新扫描、导入审批、promote、取消、删除和手动 cleanup；当前删除 / cleanup 已补齐 dry-run、confirm_token、状态白名单和本地仓库已安装包禁删。
- v1.6.0-P0-03：签名从草案升级为 Ed25519 真实验签，后台可维护可信发布者公钥；promote / install / approval 执行前会重新验签。
- v1.8.4-S9：后台“创建插件模板 / 初始化插件包”表单优化。插件名称可自动生成 `code`，高级设置可手动修改；新增插件类型（内容型、外部服务型、后台工具型、前端挂载型）并按类型动态显示字段；内容字段改为“内容数据类型 / 内容显示名称”，仅内容型插件展示；发布者 / 作者改为下拉；preview 展示 code、content_type、权限、菜单、migrations、文件树和冲突。后台、CLI 和 zip export 继续共用 `PluginTemplateGenerator`。
...
- 后台支持初始化声明型插件包模板，并把模板写入受控 draft 目录。
...
- external_service 配置可在后台保存 endpoint、health_check_path、timeout_ms、failure_policy、auth_type 与 token_ref；v1.8.4-S14 起 bearer token 由 Core SecretCenter 保存为 `secret://external_service/{plugin_code}/token`，插件包不得携带真实 token。
...
- external_service token 与 Webhook Secret、Callback Token 分离管理，列表 / 详情 / 执行记录 / 审计不回显明文；v1.9.0-S2 起 SecretCenter 和审计会提供脱敏来源字段，便于回到来源配置轮换或排查。
...
- 官方生产化插件包：`examples/plugins/official_links/`。该包从 v1.8.4-S1 起作为 DevHub 官方友情链接插件维护，声明 `friend_link` content_type、权限、后台 / 前台菜单、配置 schema、`checksums.json` 和 `migrations/001_init.sql` no-op 迁移计划文件。业务数据复用 Core `topics`，不创建私有表，不包含运行时代码、远程 iframe、远程 JS、package scripts 或真实 secret。
...
## 插件包打包 / 校验 CLI（v1.8.4-S4）

本地开发者可以先在仓库内生成和校验插件包，再进入后台 upload -> precheck -> promote -> install dry-run -> install 链路。
...
./scripts/plugin-package-build.sh examples/plugins/templates/declarative-content
...
- `passed`：没有 blocker，也没有需要管理员关注的 warning。
- `warning`：可继续本地校验，退出码仍为 0；常见原因是未签名、缺少 publisher、目录名与 `manifest.code` 不一致或存在 deprecated 根目录 SQL。建议修复后再上传生产环境。
- `blocked`：退出码为 1，禁止进入后续 upload / promote / install dry-run；常见原因是缺失 / 非法 manifest、checksum mismatch、危险文件、package scripts、可执行二进制、未知 frontend_mount、远程 iframe / 远程 JS / inline HTML、external_service 字段非法。
- 用法错误或缺少 `go` / `docker` / `python3` 等本地工具时退出码为 2。

CLI 安全边界：

- build 只复制插件包文件、重算 `checksums.json` 并生成 zip，不执行插件包代码、不执行 package scripts、不访问远程市场。
- check 复用 DevHub package dry-run / manifest 校验逻辑；不执行 SQL、不修改数据库、不启用插件、不改变 PluginRegistry 运行态、不调用 external_service、不动态加载资产。
...
后台入口：插件包治理 -> 初始化插件包 / 创建插件模板。

- 插件名称填写后会自动生成 `plugin_code`；规则为英文小写、空格 / 分隔符转下划线、移除非法字符，仅允许小写字母、数字、下划线和中划线；中文或无法转 ASCII 的名称使用 `plugin_` + 稳定短 hash 兜底。管理员可在高级设置中手动修改 code，手动修改后不会被名称继续覆盖。
- 保留 code 包含 `core`、`qa`、`docs`、`wiki`、`official_links`、`official_announcement`、`official_webhook_notify`；preview / generate 会检查 installed 插件、本地仓库、draft 目录、保留 code、content_type、权限、菜单、路由和前端挂载冲突，冲突以中文 `conflicts` 返回。
- 插件类型支持内容型插件、外部服务型插件、后台工具型插件、前端挂载型插件。字段随类型动态显示，隐藏字段不会提交到 preview / generate。
- “内容类型”已改为“内容数据类型”，只在内容型插件展示，用于系统内部识别内容数据；“内容类型名”已改为“内容显示名称”，默认跟随插件名称生成，可手动修改。
- “作者”已改为“发布者 / 作者”，支持 DevHub Team、当前组织、当前用户、可信发布者和自定义高级选项；最终 author 写入 manifest 和 preview summary。
- preview 返回自动生成的 code、content_type（如适用）、内容显示名称、权限、菜单、Hook、external_service、frontend_mount、migrations、文件树、风险提示和命名冲突；preview 不落盘、不创建插件记录、不安装、不启用、不执行 SQL、不执行代码、不动态加载资产。
- generate 固定写入 `storage/plugins/drafts/{plugin_code}/`，不允许用户指定任意生成路径，不自动 install / enable / promote。后台和 CLI 继续共用 `internal/plugins/scaffold.PluginTemplateGenerator`。
- export zip 使用同一套生成器，只生成声明型模板下载；zip 不包含 `001_schema.sql`、`.go`、可执行 JS、`.wasm`、二进制或 package scripts。
- 新模板只生成 `migrations/001_init.sql`；根目录 `001_schema.sql` 已废弃，`migrations/` 仍是唯一迁移入口，生成模板和 dry-run 均不执行 SQL。

安全边界：创建模板仍不支持代码型插件模板，不执行第三方代码，不开放 Go plugin、JS 沙箱、远程 iframe、remote component、插件市场、远程在线安装或 blocking Hook，不改变 Webhook Secret / Callback Token 安全模型。

v1.8.4-S1 official_links 官方包补充：

- 路径：`examples/plugins/official_links/`。
- 包结构：`manifest.json`、`README.md`、`config.example.json`、`checksums.json`、`migrations/001_init.sql`、`migrations/README.md`。
- `migrations/001_init.sql` 是唯一迁移入口中的 no-op 计划文件，package dry-run 会列入 `migration_plan` 且 `will_execute=false`；当前不需要插件私有业务表。
- 安装后插件默认 disabled，需要管理员显式 enable，再按子站启用；配置关闭或子站 disabled 时前台入口不展示，历史内容仍可读。
- 后台入口 `/admin-next/official-links` 复用通用 `PluginContent`，业务数据写入 Core `topics`，不绕过权限、审计、插件状态或板块绑定校验。

## 本地仓库测试包 / 未安装包批量清理（v1.8.4-S8）

后台插件包治理 -> 本地仓库增加批量清理入口：

- 清理测试包。
- 清理未安装包。
- 清理 blocked / invalid 包。

清理前必须先执行 preview：`POST /api/v1/admin/plugins/packages/cleanup/preview`。preview 只返回候选、skipped 明细、状态分布、文件数量和估算释放空间，不删除数据库记录或 storage 文件。执行清理使用 `POST /api/v1/admin/plugins/packages/cleanup` 或兼容别名 `/plugins/packages/repository/cleanup`，必须携带 preview 生成的 `confirm_token`；执行前服务端会重新扫描和重新校验，不信任前端缓存。

默认允许清理：

- `plugin_code` 前缀为 `e2e_`、`fixture_`、`test_`、`demo_`，或请求中显式给出的测试前缀。
- package name / display name / path 包含 `e2e_upload_`、`e2e_upload_promote_`、`e2e_upload_lifecycle_`、`fixture_`。
- 本地仓库中未被 installed 插件引用的包。
- `blocked`、`invalid`、`warning`、`promoted` / `ok` 但未安装的包。
- 过期 dry-run 计划关联的未安装测试包（按本地仓库可识别状态和测试前缀处理）。

禁止清理：

- installed 包。
- enabled / running 插件当前使用包。
- disabled 但已安装的插件当前版本包。
- archived 但仍有安装记录绑定的当前版本包，除非另走卸载 / 归档清理流程。
- 有 active enable / upgrade / uninstall task 的包。
- 非测试前缀且非 blocked / invalid 的正式包，例如 `official_links`、`official_announcement`、`official_webhook_notify` 等正式包，除非它们明确使用 fixture / e2e 测试前缀。

执行清理会删除两类内容：`storage/plugins/packages/` 下对应包目录，以及对应 promoted upload 本地仓库记录。它不会删除插件安装记录、插件配置、历史内容、`admin_logs`、`hook_executions` 或 Webhook / Callback Token 元数据。storage 删除只允许发生在 `storage/plugins/packages/` 子目录，服务端会对路径做 clean / abs / prefix 校验，拒绝路径穿越、仓库根目录和任意用户输入路径；文件不存在只记录 warning，不导致整体失败。

清理动作写 `admin_logs`：`plugin.package.cleanup.preview`、`plugin.package.cleanup.started`、`plugin.package.cleanup.success`、`plugin.package.cleanup.partial_failed`、`plugin.package.cleanup.failed`、`plugin.package.cleanup.skipped_installed`、`plugin.package.cleanup.skipped_enabled`。metadata 只记录 scope、prefixes、计数、plugin_codes 和 storage path hash，不记录 token / secret / Authorization、包内敏感内容或 SQL 全文。

该能力仅清理本地测试 / 开发数据，不执行第三方代码，不执行 SQL，不刷新或破坏 PluginRegistry 当前运行态，不影响 upload / precheck / promote / install dry-run / install 主链路。MemoryStore 与 MySQLStore 通过同一套 Service 规则和 Store 接口删除 promoted upload 记录；生产环境不建议手工直接删数据库。

> 注：v1.5.0 当前支持本地插件仓库扫描、插件包详情、dry-run、安装确认与审批执行链路；安装仍走 `plugin.approve` 执行权限，不允许携带代码或可执行资产的插件包。

## 生产安装 / 升级前检查（v1.8.4-S5）

插件包 build / check 只是本地准备，不等于可以直接进入生产安装或升级。管理员在执行 upload、precheck、promote、install dry-run、install 或 upgrade 前，建议至少完成以下检查：

- 当前数据库已备份，且备份已在临时库验证可恢复。
- 插件安装记录、插件配置记录、插件配置加密 key、本地插件仓库、上传暂存区、`admin_logs`、`hook_executions`、Webhook Secret 元数据、Callback Token 元数据、external_service 配置、站点 / 子站插件启用状态和 migrations 执行记录都可以恢复。
- `scripts/plugin-package-check.sh` 没有 blocker；warning 的含义已理解，适合继续本地或预发演练，不建议直接跳过进入生产。
- install dry-run 已从本地插件仓库包重新执行；upload / staging 阶段旧 dry-run 不能替代 install dry-run。
- `install dry-run` / `upgrade dry-run` 的 plan 没有过期，且 checksum、manifest、plugin code、version、migration plan hash 与当前包一致。
- `migrations/` 计划已经确认，不依赖根目录 `001_schema.sql`；dry-run 仍不执行 SQL、不写安装状态、不刷新 PluginRegistry。
- 插件包不包含 package scripts、可执行二进制、远程 iframe、remote JS、remote component、inline HTML 或未允许的前端执行入口。
- 升级前已经看过 manifest / permissions / content_types / config_schema / frontend_mounts / hooks / external_service / menus / routes / migrations / dependencies 的差异。
- 影响站点 / 子站数量、权限、菜单和外部服务投递影响已经确认。
- `failure_stage`、`failure_reason` 和 `PluginRegistry reload` 失败处理路径已经明确。
- 对于会改库的插件，必须先做 MySQL 备份；`MemoryStore` 只适合本地开发 / 临时测试，生产建议使用 MySQLStore。

升级风险等级处理：

- `safe`：可继续审批 / 执行，但生产仍需先完成备份。
- `warning`：必须由管理员显式 `confirm=true`，并记录接受原因。
- `blocked`：不能通过 `confirm=true`、审批或前端按钮绕过；必须修复包、依赖、Core 兼容、checksum、frontend_mount、Hook 或 migration plan 后重新 dry-run。

恢复边界保持不变：

- 当前不提供完整自动 rollback。
- 当前不提供 migration down。
- install / upgrade 失败后优先使用 dry-run、审计、备份和人工恢复路径。
- 如果 migration 已执行，默认不承诺自动回退；应优先恢复数据库备份或由 DBA 人工处理。
- 配置恢复依赖配置记录、配置版本历史和同一组插件配置加密 key；加密 key 丢失后加密配置无法恢复。
- 插件包本地仓库可以通过备份目录恢复，或者重新 promote 原 zip。
- `disabled` / `archived` 只是治理状态切换，不等同于完整回滚。

完整生产备份清单、安装 / 升级前检查、`failure_stage` 处置矩阵、`PluginRegistry reload` 失败处理、MySQLStore 生产验收和演练步骤见 [备份与回滚文档](BACKUP_AND_ROLLBACK.md)。
...
- v1.8.4-S11 补充：external_service 运行配置与插件全局 `config_json` 是两套配置；Webhook 健康检查和投递只读取 external_service 运行配置。已安装同 code 插件不能重复 install，应进入插件详情配置、external_service 配置、版本仓库升级差异或审批流程。feishu_link 独立本地 receiver 推荐端口为 `18081`；`cmd/webhook-mock-receiver` 默认 `18090`，是另一套官方 mock receiver。
...
- 输出目录固定为 `storage/plugins/drafts/{code}`。
- `code` 必须满足小写字母、数字、下划线或中划线并以小写字母开头；内容型插件的 `content_type` 必须满足小写字母、数字、下划线并以小写字母开头。
- 不允许任意 path。
- 不暴露 `force`；非空目标目录已存在时返回 `plugin_package_template_exists`，空目录可复用。
- 初始化成功后写入 `admin_logs`。
...
- 后台 preview、generate、export zip 和 CLI `plugin:new` 共用 `internal/plugins/scaffold.PluginTemplateGenerator`。
- 所有入口都不生成 `registry.example.go`，避免 `.go` 文件触发插件包扫描器的 dangerous file blocked。
- registry 接入说明统一放在 `docs/registry-example.md`。
- 模板只生成 `migrations/001_init.sql`，不生成根目录 `001_schema.sql`。
- dry-run 未 blocked 时，后台允许继续提交“安装审批”；直接安装仍需 `plugin.approve`。

边界：不做远程市场 / 远程下载；不执行插件代码；不执行 SQL；不生成 Go/JS/WASM/binary/package scripts；不生成 blocking Hook；不生成前端动态页面；不改变插件运行时边界。
...
uploaded/staged/blocked/failed/expired -> deleted（删除文件与上传记录；审计保留在 admin_logs）
...
- 上传记录删除仅允许 `uploaded/staged/blocked/failed/expired`；`promoted/installed/approval_pending/install_approval_pending/canceled/deleted` 不允许直接删除。
- `deleted` 不支持恢复；需要重新上传。已 promote 的本地仓库包必须走本地仓库删除规则，已安装插件必须先归档 / 软卸载。
...
- `DELETE /api/v1/admin/plugins/packages/uploads/:upload_id`：仅删除 `uploaded/staged/blocked/failed/expired` 的上传 / staging / quarantine 文件和上传记录；审计记录保留。
- `POST /api/v1/admin/plugins/packages/uploads/cleanup`：支持 `dry_run`、`confirm_token`、`statuses`、`older_than_days`，默认预览并清理 `blocked/failed/expired`；执行前必须先 dry-run。
- `DELETE /api/v1/admin/plugins/packages`（别名：`/plugins/packages/repository`）：删除 `storage/plugins/packages/` 下未安装的本地仓库包；已安装包拒绝并提示先归档 / 软卸载。
- `POST /api/v1/admin/plugins/packages/cleanup`（别名：`/plugins/packages/repository/cleanup`）：支持 dry-run / confirm_token 批量清理本地仓库 `blocked/invalid`，只处理未安装包。

cleanup 响应统一返回：`will_delete_count`、`will_free_bytes`、`items`，确认执行后返回 `deleted_count`、`freed_bytes`。删除路径始终限制在 `storage/plugins/uploads`、`storage/plugins/staging`、`storage/plugins/quarantine`、`storage/plugins/packages`，服务端会阻断路径穿越和白名单外路径。
...
页面能力：上传 zip、筛选列表、详情抽屉、zip scan、package scan、checksum、signature、risk_report、manifest validate、dry-run、可执行动作与不可用原因、rescan、提交导入审批、审批通过 / 拒绝、promote、cancel、delete、cleanup。插件包治理页的本地仓库 Tab 也提供未安装 promoted 包删除和 blocked/invalid cleanup；已安装包删除按钮禁用，并提示先归档 / 软卸载插件。页面明确展示：上传包只是 staging，promote 不等于安装，不执行插件代码、不执行 SQL、不加载前端资产。
...
- `config.example.json`
- `checksums.json`
- `PACKAGING.md`（打包说明，可选）
- `package.example.md`（示例说明，可选）
- `receiver.example.md`（external_service receiver 示例，可选）
- `publisher.json`（发布者信息，可选）
...
## 如何消除 medium 告警（unknown_files / checksum_missing）

`risk_report.level=medium` 典型来自两类原因：

1. 缺少 `checksums.json`（`plugin_package_checksum_missing`）。
2. 包内存在未知文件（`plugin_package_unknown_files` / `unknown_files`）。

建议按下面顺序补齐：

1. **先补齐 `checksums.json`**：让包的“文件完整性”可验证。
2. **再处理未知文件**：把需要的说明文件放进允许位置，其他文件删掉或移出包目录。

### 1) 生成或更新 checksums.json

最简单方式是用仓库脚本生成 zip（脚本会在构建时重算 `checksums.json` 并写入 zip）：

```bash
./scripts/plugin-package-build.sh examples/plugins/<your_plugin_dir>
./scripts/plugin-package-check.sh examples/plugins/<your_plugin_dir>
```

如果你希望把 `checksums.json` 直接落在源码目录里（便于提交到 Git），可以按 `checksums.json` 的规则自行生成并写回目录（algorithm=sha256，覆盖所有需要交付的文件，且不包含 `checksums.json` 自身）。

### 2) 处理 unknown_files（未知文件）

未知文件不会直接阻断 dry-run，但会让风险等级升到 `medium`。常见处理方式：

- 文档类文件：放到 `docs/*.md`，或使用根目录允许的固定文件名（如 `PACKAGING.md`、`receiver.example.md`）。
- 示例类文件：放到 `examples/*.md|.json`。
- 图片类文件：放到 `assets/`，并使用 `png/jpg/jpeg/webp`；`assets/*.svg` 当前仍是 unknown，建议先转换为位图或移到 `docs/` 作为展示说明。
- 旧的根目录 `001_schema.sql`：属于历史兼容输入，当前不会执行；如不需要建议移除（真实迁移只能放 `migrations/`）。

当 dry-run 返回 `unknown_files` 时，优先遵循“最小可交付包”原则：只保留 manifest、配置示例、必要文档、迁移计划与 assets，其余文件建议移出包目录。
...
- Core：`compatible_core_version` 必须存在，当前 Core 来自 `VERSION`（当前口径为 `v1.9.0`）。支持 `1.9.0`、`>=1.9.0`、`>=1.9.0 <2.0.0`、`^1.9.0`、`~1.9.0`；不支持复杂预发布标签、`||` 或 npm 全量 semver。
