...
`VERSION` 当前为 `v1.9.0`；v1.9.0 定义为“官方插件生态稳定版”，已完成发布归档与发布后 targeted smoke，P0=0，P1=0，P2=0，版本可以冻结。建议先提交当前文档归档，再创建 v1.9.0 annotated tag。v1.9.0 冻结后，项目进入下一阶段规划；不再向 v1.9.0 追加新功能。当前实现已从早期插件包治理 / 签名验签 / 运行模型设计推进到后台插件治理稳定性、声明型插件真实业务闭环、插件包真实 zip 验收、PluginRegistry reload、external_service non-blocking Webhook、SecretCenter token_ref、MySQLStore 生产流程和发布归档收口。远程包、声明型插件和 external_service 仍不执行第三方代码、不开放动态加载、不开放远程 iframe、不做 blocking Hook。
...
- 已完成：v1.8.4-S5 生产备份 / 回滚 / 升级演练文档增强。已补齐生产备份清单、插件安装 / 升级前检查、`safe / warning / blocked` 风险等级、warning 必须 `confirm=true`、blocked 不能绕过、`failure_stage` / `failure_reason` 处理建议、`PluginRegistry reload` 失败路径、配置与加密 key 恢复、本地仓库恢复、Webhook Secret / Callback Token / external_service 元数据备份、MySQLStore 生产建议与 MemoryStore 限制；当前仍不承诺完整自动 rollback 或 migration down，不改变插件运行能力。
- 已完成：v1.8.4-S8 插件包本地仓库测试数据批量清理。插件包治理 -> 本地仓库支持清理测试包、未安装包、blocked / invalid 包；新增 cleanup preview，执行前重新校验，删除未安装包的 storage 目录和 promoted upload 记录，installed / enabled / active task 包跳过并写审计。
- 已完成：v1.8.4-S9 后台初始化插件包表单优化。插件名称自动生成 code（小写字母、数字、下划线、中划线），高级设置可手动修改；新增内容型 / 外部服务型 / 后台工具型 / 前端挂载型插件类型，字段动态显示；内容字段改为“内容数据类型 / 内容显示名称”；发布者 / 作者改为下拉；preview 展示自动生成字段、权限、菜单、migrations、文件树和 conflicts；generate 写入 `storage/plugins/drafts/{code}/`，后台、CLI、export 继续共用 `PluginTemplateGenerator`。
- 已完成：v1.9.0-S6 插件模板生成稳定化。CLI 补齐四类插件模板参数并与后台 preview / generate / export zip 共用 `PluginTemplateGenerator`；输出统一为声明型文件、`docs/*.md` 和 `migrations/001_init.sql`，不生成 `registry.example.go`、根目录 `001_schema.sql`、可执行代码、package scripts 或 blocking Hook 模板。
- 已完成：v1.8.4-S13 系统配置中心与敏感配置治理收口。新增系统敏感配置只读状态接口（启动加密 keyring 状态 + external_service HTTP allowlist），系统设置页展示只读状态与 env 示例；Webhook Secret 创建前增加 readiness 检查并补齐缺钥错误引导，明确 root key 只能来自启动环境变量、后台不保存或生成、修改需重启；后台对 `secret_ref/token_ref` 做脱敏展示（可复制引用，不铺开明文）。
- 已完成：v1.8.4-S14 Core SecretCenter 最小引用层与 external_service token_ref 落地。新增 `secret_refs` 存储表与 `secret://{namespace}/{name}` 引用规范；新增 SecretCenter 元数据管理 API（不提供明文读取）；external_service bearer token 写入时统一落到 SecretCenter 并绑定 `token_ref=secret://external_service/{plugin_code}/token`，运行时 resolve token_ref（仅内部使用），API/审计/执行记录不回显明文；查询和保存响应仅返回 `token_ref` / `token_secret` 元数据；系统只读敏感配置状态接口增加 SecretCenter 统计。
- 已完成：v1.8.4-S15 SecretCenter + Webhook + external_service MySQL 实跑验收。Docker Go + MySQLStore + feishu_link receiver 下已验证 SecretCenter 直接写入/轮换、external_service bearer token_ref、health check、真实 non-blocking 投递、Webhook Secret 创建/轮换、Callback Token 回调 config/audit、MySQL 密文落库和审计/执行记录脱敏；并修复 MySQLStore Webhook Secret / Callback Token insert 占位数量不匹配问题。
- 已完成：v1.8.4-S19 SecretCenter 操作闭环与当前生效配置排障入口收口。SecretCenter 详情、使用关系、来源跳转、禁用 / 吊销影响预览、吊销强确认、危险操作权限和 admin_logs 链接已接入；关系解析基于 external_service / Webhook Secret / Callback Token 的真实存储记录，轮换入口跳回来源治理，不回显敏感明文，不改变加密或 Webhook / Callback 安全模型。
- 已完成：v1.9.0-S2 SecretCenter 操作闭环补强。SecretCenter 元数据、详情、使用关系和审计 metadata 增加 `usage_type/source_type/source_id/source_code` 脱敏来源字段；当前生效配置增加 config source、token source 和 next_steps 排障建议；未知 / 测试来源不提供误导性跳转或危险操作，轮换仍只回到来源配置。
- 已完成：v1.9.0-S3 当前生效配置与排障视图稳定化。effective-config 聚合响应和系统设置页补齐基础运行信息、root key 只读状态、SecretCenter 状态、HTTP Allowlist 来源 / endpoint origin 匹配、external_service 非敏感字段、`token_ref` 脱敏来源元数据、Webhook / Callback 安全摘要、redacted `diagnostic_text` copy 和 quick links；不改变 SecretCenter 加密逻辑、Webhook Secret / Callback Token 模型或 external_service non-blocking delivery。
- 已完成：v1.9.0-S1 fresh receiver 全链路回归。MySQLStore + standalone receiver 覆盖 `feishu_link` success、500 failure、timeout 和 manual retry；确认 `hook_executions`、`admin_logs`、`token_ref`、SecretCenter 存储和 DB/API 敏感扫描通过。该项清掉 v1.8.4 P2，但不新增插件运行能力。
- 已完成：v1.9.0-S11 发布归档与发布后 smoke。`VERSION`、Release Notes、CHANGELOG、README、PROJECT_PROGRESS 和 TESTING 已统一到 `v1.9.0` 官方插件生态稳定版口径；admin quick 与后台 IA smoke 通过，最终 P0=0、P1=0、P2=0。版本可以冻结，建议先提交当前文档归档，再创建 v1.9.0 annotated tag。
...
下一阶段建议优先级：v1.9.1 可优先做发布后小修、生产备份演练自动化、release smoke 脚本固化、文档口径巡检、测试补强和兼容性补丁；v1.10.0 再把 `docs/PLUGIN_RUNTIME_MODEL.md` 中的前端挂载、HTTP 插件服务协议、受控 API token 和隔离边界拆成实现任务，并推进更多官方示例插件验证和远程分发前置治理。动态 Go 加载、脚本沙箱、第三方代码执行、完整插件市场交易 / 评分 / 评论系统继续后置。
...
- 先问任务，再给入口。管理员先明确“我要做什么”，再落到治理域、页内 Tab、抽屉或跳转页。
- 高频入口保留在总览、列表和主治理页，低频入口可以沉到高级治理或技术详情，但不能变成只剩旧路由才找得到的隐藏项。
- 详情抽屉只负责单插件摘要、快捷跳转和最小排障分组，不承担全局控制台职责。
- 技术详情只保留排障用途的最小信息：启用检查、迁移、导出、原始声明 / JSON。
- 新入口若无法在一句话里说明“做什么、去哪做”，就先不要进入左侧导航。

任务 -> 入口：

- 日常状态查看和单插件处理 -> 插件总览 / 插件列表 / 插件详情抽屉。
- 安装、预检、promote、dry-run、安装计划 -> 插件包治理。
- Webhook 投递、异常、密钥、回调 Token、回调请求 -> Webhook 治理。
- 发布者、公钥、可信性、影响分析 -> 发布者与信任。
- 操作历史、审计、Hook 排障、最近错误 -> 运行记录 / 审计。
- 原始声明、配置模型、迁移明细、导出包结构 -> 插件详情技术详情。
...
本节保留为 v1.3.5 草案与阶段目标追溯；当前版本线以本文顶部“当前实现快照与需求分层”和 `docs/PROJECT_PROGRESS.md` 为准。
