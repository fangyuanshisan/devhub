...
`VERSION` 当前仍为 `v1.3.4`。代码工作区已经包含 `v1.3.5` 插件治理体验改动：内置插件治理、ManifestValidator、manifest dry-run、manifest + 配置型安装、最小升级执行、归档 / 恢复、健康总览、批量归档 / 恢复、Hook 失败注入、迁移失败阻断、权限矩阵、MySQLStore 专项、后台治理中心重排、安装 / 升级抽屉分步向导、状态治理视图和最小后台 E2E 均已具备基础闭环。
...
本阶段只收口现有插件治理体验，不引入危险运行时能力。当前主体功能已在工作区实现，下一步是校准口径、补验收、处理 skip、准备发版。

P0 目标状态：

1. `/admin-next/plugins` 信息架构重排：已完成后台最小可用重排。
...
7. 最小 E2E：已覆盖新入口和核心安全确认；剩余工作是解释或处理 skipped 用例。
