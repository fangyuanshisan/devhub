...
v1.7 后续建议优先级：插件包解压安全检查与 manifest 预校验、插件安装事务与回滚、插件启用与运行时注册（需依赖 enable-precheck）、远程索引缓存刷新策略、trusted publishers 远程只读同步草案、zip export 下载能力、上传 / 下载任务队列、操作快照增强、插件包安全测试 fixture 生成器。动态运行时、脚本沙箱和第三方代码执行继续后置。
