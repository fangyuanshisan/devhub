...
## 当前新增阶段：阶段 C/D/E/F 最小交付

阶段 C/D/E/F 在 v1.3.4 后继续沿完整插件平台主线推进，但仍不实现插件市场、外部插件上传安装、远程安装、在线更新、Go 动态加载、第三方插件沙箱或硬卸载。

- 阶段 C：新增 `docs/plugins/` 插件 SDK / 模板规范，包括 manifest 示例、插件目录模板、配置、Hook、迁移、权限、菜单路由和外部生态设计入口。
- 阶段 D：内置插件建立可展示的安装生命周期模型，返回 `install_status`、`lifecycle_status`、`status_reason`、`installed_at`、`archived_at`、`last_health_check_at` 等派生字段；启用前检查归档、配置、依赖和 failed migration。
...
## 当前阶段需求：v1.3.4 插件迁移与 Hook 失败注入验收闭环
