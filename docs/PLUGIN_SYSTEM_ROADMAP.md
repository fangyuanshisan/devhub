...
`v1.3.4` 已把插件异常治理和平台基础能力收口到可用基线：内置插件治理、ManifestValidator、manifest dry-run、manifest + 配置型安装、最小升级执行、归档 / 恢复、健康总览、批量归档 / 恢复、Hook 失败注入、迁移失败阻断、权限矩阵和 MySQLStore 专项均已具备基础闭环。
...
- 部分完成：Hook 超时 / failure_policy 已有 manifest 字段和运行记录基础，但外部服务 Webhook 尚未接入真实 HTTP 调用；迁移 runner 仍是内置 up/no-op 与记录型迁移，不执行外部 raw SQL；安装 / 升级当前是结构化工作面板，不是完整分步向导。
- 仍未完成：完整插件治理 UI 信息架构、完整安装向导、完整升级向导、批量归档 / 恢复影响预览表格、状态治理页、插件包 zip 上传、外部服务型 Webhook 执行、插件包签名、远程市场、动态加载、脚本沙箱、硬卸载和 migration down。

## 下一阶段：v1.3.5 插件治理体验与安装升级向导收口

本阶段只收口现有插件治理体验，不引入危险运行时能力。

P0 目标：

1. `/admin-next/plugins` 信息架构重排：总览层、筛选与批量操作层、插件列表层、详情治理层。
2. 完整安装向导：Manifest 输入、校验结果、dry-run 影响预览、确认安装。
3. 完整升级向导：Manifest 输入、版本兼容矩阵、变更影响预览、确认执行升级。
4. 批量归档 / 恢复体验：影响预览、成功 / 失败明细表、审计跳转。
5. 状态治理页：作为异常处理入口，聚合迁移待处理、Hook 异常、配置无效、依赖缺失和归档插件。
6. PluginContent 体验对齐：归档 / 禁用插件下历史内容仍可治理，新建入口明确不可用。
7. 最小 E2E：只覆盖上述新入口和核心安全确认，不扩成完整浏览器矩阵。
