...
`VERSION` 当前为 `v1.6.0`，代码能力已完成插件包上传与分发前置能力收口：zip 上传安全沙箱、上传包生命周期、Ed25519 真实验签、可信发布者管理、远程插件索引只读镜像、插件包版本仓库、升级差异、操作快照 / 失败恢复预览、配置密钥轮换和后台插件治理 UI 分组。
...
- 仍未完成 / 后置：远程插件包下载、远程安装、在线更新、自动安装依赖、远程可信源同步、完整 PKI / CA、动态加载、脚本沙箱、第三方代码执行、外部 raw SQL 自动执行、hard uninstall、migration down、完整远程插件市场交易 / 评分 / 评论系统。

v1.7 建议优先级：远程插件包下载到 staging、下载审批与安全校验、远程索引缓存刷新策略、trusted publishers 远程只读同步草案、zip export 下载能力、上传 / 下载任务队列、操作快照增强、插件包安全测试 fixture 生成器、插件治理 UI 可用性继续优化和配置密钥轮换自动预检。动态运行时、脚本沙箱和第三方代码执行继续后置。

## 历史收尾：v1.3.5 插件治理体验与安装升级向导

本节保留为 v1.3.5 草案与阶段目标追溯；当前版本线以 `v1.6.0` 的验收记录与 `docs/PROJECT_PROGRESS.md` 为准。
