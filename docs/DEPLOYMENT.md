...
默认端口为 `8090`，`dev.sh` 默认数据仓库为 `CMS_STORE=mysql`，会启动开发 MySQL 并使用 `devhub_mysql_data` volume 保留插件安装信息。`main.go` 直接运行也默认监听 `8090`，且未设置 `CMS_STORE=memory` 时默认连接 MySQL；仍可通过 `PORT=8080 go run .` 覆盖端口。修改 Go 后端代码后需要重启服务；只改 Go 且前后台产物无需重建时，可以使用 `./dev.sh restart --no-build` 或 `./dev.sh --local-go restart --no-build`。
...
在执行插件安装、升级、启停或归档前，建议先完成：

- 主数据库备份，并在临时库验证可恢复。
- 插件安装记录、插件配置记录、插件配置加密 key、Core SecretCenter `secret_refs`、插件包本地仓库、上传暂存区、`admin_logs`、`hook_executions`、Webhook Secret 元数据、Callback Token 元数据、external_service 配置、站点 / 子站插件启用状态和 migrations 执行记录备份。
- 确认加密 key 已安全备份；key 丢失后加密配置无法恢复。
- 确认 token / secret 明文没有进入日志、审计、备份摘要或不安全导出目录。
- `scripts/plugin-package-check.sh`、precheck、install dry-run 和 upgrade dry-run 的本地 / 预发预演。
- 确认 install dry-run / upgrade dry-run plan 未过期，且 checksum、manifest、plugin code、version、migration plan 与当前包一致。
- 确认当前是 `safe`、`warning` 还是 `blocked`；`warning` 需要显式 `confirm=true`，`blocked` 不可绕过。
- 确认 `failure_stage`、`failure_reason`、`PluginRegistry reload` 失败处理路径、配置恢复路径和本地仓库恢复路径已经明确。
- MySQLStore 作为生产建议模式；MemoryStore 仅适合本地开发、临时测试或无持久化环境。

如果插件会改数据库结构，先看 `docs/BACKUP_AND_ROLLBACK.md`，再做预发 / 临时库演练，不要把自动回滚当作默认前提。

MySQL 生产模式验收至少覆盖：插件安装、配置保存、敏感字段脱敏、`admin_logs` 查询、`hook_executions` 查询、upgrade dry-run safe / warning / blocked、dry-run plan 过期拒绝、checksum 不一致拒绝、migration plan 不一致拒绝和 PluginRegistry reload 成功路径。
