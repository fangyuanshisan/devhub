...
DevHub 当前长期目标是 **Core + 插件 的开源服务底座**。插件系统是 DevHub 的核心方向，不是附属模块。Core 提供稳定基础能力，插件承载业务扩展、前后台页面扩展、Hook、配置、内容类型、第三方集成和垂直功能扩展。
...
官方插件生态稳定补充（v1.9.0-S1）：

- v1.9.0 定位为官方插件生态稳定版，重点稳定 v1.8.4 已经落地的官方插件、external_service Webhook、SecretCenter、插件包治理、后台 IA 与 MySQLStore 生产流程，不引入新的高风险运行时能力。
- S1 已用 MySQLStore + fresh standalone receiver 补跑 `feishu_link` / `plugin_a7b0cc04` 的 success、500 failure、timeout 和 manual retry 全链路；`hook_executions`、`admin_logs`、`token_ref` 与敏感信息扫描均通过。
- 该回归不改变架构边界：external_service 仍只是受控 non-blocking HTTP 投递，不是第三方代码执行环境；不开放 Go plugin、JS sandbox、WASM、Lua sandbox、远程 iframe、remote component、插件市场、远程在线安装或 blocking Hook。

发布归档补充（v1.9.0-S11 / post-archive）：

- DevHub v1.9.0 官方插件生态稳定版已完成发布归档与发布后 targeted smoke，P0=0，P1=0，P2=0，版本可以冻结。建议先提交当前文档归档，再创建 v1.9.0 annotated tag。
- v1.9.0 冻结后，项目进入下一阶段规划；不再向 v1.9.0 追加新功能。
- v1.9.1 是维护候选，适合小修、文档、测试脚本固化和兼容性补丁；v1.10.0 是规划阶段，适合更大的功能、治理和架构演进。二者均不属于 v1.9.0 已实现范围。

后台模板表单补充（v1.8.4-S9）：

- 后台“创建插件模板 / 初始化插件包”和 CLI 继续共用 `internal/plugins/scaffold.PluginTemplateGenerator`，避免后台与 CLI 双实现。
- 后台表单新增插件类型：内容型、外部服务型、后台工具型、前端挂载型；字段按类型动态显示。内容字段改名为“内容数据类型 / 内容显示名称”，只在内容型插件展示。
- 插件名称会自动生成 code（小写字母、数字、下划线、中划线），发布者 / 作者改为下拉；preview 会展示自动生成字段、权限、菜单、Hook、external_service、frontend_mount、migrations 和文件树，并返回命名冲突。
- generate 固定写入 `storage/plugins/drafts/{code}/`；模板仍只包含声明、配置示例、文档和 `migrations/001_init.sql`，不生成根目录 `001_schema.sql`，不安装、不启用、不执行 SQL、不执行第三方代码。
- v1.9.0-S6 已将 CLI、后台 preview / generate / export zip 的输出进一步统一为 `docs/*.md` + `migrations/001_init.sql`，不再生成 `registry.example.go`、Go/JS/WASM/binary、package scripts 或 blocking Hook 模板。

官方插件生产化补充（v1.8.4-S1）：

- `official_links` 已从声明型样板推进为生产化官方友情链接插件包，路径为 `examples/plugins/official_links/`。
- 该插件仍是声明型插件：通过 manifest 声明 `friend_link` content_type、权限、后台 / 前台菜单、配置 schema 和 migrations/，后台管理复用通用 `PluginContent`，前台展示复用搜索和子站插件导航。
- 业务数据复用 Core `topics` / `categories` / `plugin_configs` / `admin_logs`，不创建插件私有运行时，也不执行插件包代码。
- `migrations/001_init.sql` 是 no-op 计划文件，只作为唯一迁移入口中的计划项；dry-run 不执行 SQL，install 不执行 package scripts。
- 该生产化不改变插件系统底层运行模型，不开放 Go plugin、JS 沙箱、远程 iframe、remote component、任意远程 JS、插件市场、远程在线安装或 blocking Hook。
...
- v1.8.4-S14 已落地 Core SecretCenter 最小引用层，并将 external_service bearer token 统一收口为 `token_ref=secret://external_service/{plugin_code}/token`：明文仅在写入请求中出现一次，列表/详情/保存响应/审计/执行记录不回显；health check 与投递运行时 resolve token_ref（仅内部使用），MySQL 只保存密文 `enc:v2:*` 与元数据。该能力不引入 KMS/Vault、不开放第三方代码执行，不改变 Webhook Secret / Callback Token 现有安全模型（仅补齐后续迁移口径）。
- v1.8.4-S17 补充系统配置可读视图：系统设置 -> 当前生效配置聚合展示 external_service 当前运行配置、HTTP Allowlist 来源 / 最终生效列表和 SecretCenter 脱敏元数据。endpoint_url、health_check_path、enabled、timeout_ms、failure_policy 等非敏感字段明文展示；token / secret / Authorization 只通过 token_ref / secret_ref、status、key_id、masked 和用途说明展示，并支持复制脱敏诊断 JSON。该视图不改变 SecretCenter 加密逻辑、不改变 token_ref/secret_ref 解析语义、不改变 Webhook Secret / Callback Token 安全模型。
- v1.8.4-S18 将系统设置 -> SecretCenter 页面收口为“敏感配置引用”治理页：管理员可按类型、状态和关键词查看 secret_ref / token_ref 元数据，识别 external_service、Webhook、Callback 与测试数据；`secret://...` 被明确说明为引用地址，不是敏感值本身。页面只展示元数据，不展示 token / secret / Authorization / encrypted_value / root key；轮换入口引导到来源治理，不改变 SecretCenter 加密、解析或 Webhook / Callback 安全语义。
- v1.8.4-S19 将 SecretCenter 从只读治理入口推进到操作闭环：详情抽屉基于真实存储关系展示 external_service / Webhook Secret / Callback Token 来源和使用关系；禁用 / 吊销前提供影响预览，吊销要求完整 ref 强确认，危险操作由后端权限保护；轮换入口只跳转来源治理，不在 SecretCenter 收集明文。当前生效配置页也补齐去配置、健康检查、运行记录、查看 Secret 和审计入口。安全边界不变：不回显敏感明文，不改变加密模型，不开放第三方代码执行或 blocking Hook。
- v1.9.0-S2 继续收口 SecretCenter 运维闭环：Secret 元数据新增 `usage_type/source_type/source_id/source_code` 脱敏来源字段，审计 metadata 可按 ref、namespace/name、source_type/source_id/source_code 和 plugin_code 排查；当前生效配置补充 config source、token source、next_steps 和复制脱敏诊断入口。轮换仍只回到来源配置，禁用 / 吊销仍先预览影响；root key 仍只来自启动环境变量或外部 Secret 系统，后台不保存、不生成、不修改。
- v1.9.0-S3 稳定当前生效配置与排障视图：系统设置 -> 当前生效配置按基础运行信息、SecretCenter、Webhook / Callback 安全摘要、HTTP Allowlist、external_service 和脱敏诊断文本分区展示；effective-config 响应补齐 root key 只读状态、SecretCenter 状态、HTTP Allowlist 来源、endpoint origin 匹配、external_service 非敏感字段、token_ref 脱敏来源元数据和 quick links。`diagnostic_text` 可复制且脱敏，复制失败有手动 textarea 兜底；root key 仍只启动注入，不进入后台保存、生成、复制或编辑。
- v1.8.4-S11 已完成 Webhook 本地联调体验与 external_service 配置治理收口：endpoint 安全拒绝、Docker loopback、网络超时、HTTP 状态失败、token 缺失 / 无效和插件 / 子站 / 服务停用均有 diagnostics / suggestion；插件详情概览和 Webhook 治理页提供“配置 external_service”入口；当前健康、最近成功、最近失败和历史失败计数分开展示；全局 `config_json` 中疑似 Webhook 字段会提示不会自动覆盖 external_service 运行配置。HTTP 安全默认值不变，非 localhost HTTP 仍需 `DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST` 显式放行。
...
- v1.8.3-S23 完成发布前总验收：MemoryStore 与 MySQLStore 的关键插件能力已通过 Go 全量、插件专项、前后台 quick 和 MySQL 模式 smoke 复查。MySQLStore 新装 schema 中 Webhook Secret / Circuit Breaker 相关表的 indexed `target_url` 已收敛为 512 字符，避免 utf8mb4 联合索引超长；Webhook Secret 创建也会在服务层用中文错误阻断超长 `target_url`。该修复只影响索引字段长度和输入校验，不改变 Webhook 协议、Secret / Token 安全模型或 external_service 投递语义。
...
- 用户账号、前台认证、后台认证、子站版主授权。
- 角色权限、后台 RBAC、版主 scope 校验。
- 内容基础模型、分类、标签、评论、搜索、通知、收藏、关注、举报。
- 后台管理基础框架、API 基础能力、审计记录和安全边界。
- `/topics/:id`、子站页、标签页、sitemap 和 robots 的 SEO 兜底。
- 插件生命周期、插件包治理、插件权限治理、插件配置治理、HookBus，以及安装、启用、停用、归档、恢复等基础流程。

插件层职责：

- 业务功能扩展、垂直业务能力、第三方服务集成。
- 自定义内容类型、自定义配置表单、权限、菜单、路由声明和 Hook 监听。
...
当前 `VERSION` 为 `v1.9.0`，主题是“官方插件生态稳定版”。在 v1.7 / v1.8 插件包治理、签名验签、声明型插件闭环、PluginRegistry reload、external_service non-blocking Webhook、SecretCenter `token_ref`、后台 IA 和 MySQLStore 生产模式基础上，v1.9.0 已完成发布候选总验收、发布归档与发布后 smoke，最终分级为 P0=0、P1=0、P2=0。

下一阶段建议围绕 v1.9.1 / v1.10.0 继续推进生产备份演练自动化、release smoke 脚本固化、更多官方声明型插件样例、远程分发前置治理和完整第三方运行模型设计拆分；仍不引入动态执行环境、第三方代码执行、远程 iframe、插件市场、远程在线安装或 blocking Hook。
...
- 当前已支持 manifest + 配置型插件的校验、dry-run、插件包治理、远程 staging 下载、签名验签、安装 / 启用 / 软卸载 / 升级治理和最小升级执行闭环；仍不支持插件市场、远程自动安装、在线更新、Go 动态插件加载或执行第三方本地代码。这些能力进入后续路线，推进时必须满足安全红线、权限隔离、migration 备份回滚和 SEO 不退化。
...
- 系统配置与敏感配置的三层边界（v1.8.4-S13 口径）：
  - 启动/根密钥（root secrets）：只能来自环境变量或外部 Secret 系统（KMS/Vault/Docker Secret/K8s Secret 等）注入到环境变量；DevHub 后台不会保存或生成。
  - 运行时敏感配置：例如 Webhook Secret、Callback Token、external_service token 等，必须由 Core 受控写入并使用启动 root key 加密入库；API/日志/审计不回显明文。
  - 普通 Key/Value 配置：例如系统设置、插件非敏感配置等，可按现有 Core 配置与插件配置中心写入数据库，但不得承载 root secrets。
