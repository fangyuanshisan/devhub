...
当前 `VERSION` 为 `v1.4.0`，主题是“插件内容治理增强版”。历史阶段 `v1.3.4` / `v1.3.5` 已完成插件平台基础能力与治理中心体验收口；在此基础上，`v1.4.0` 进一步补齐 `PluginContent` 的精确 `plugin_code + content_type` 过滤、禁用 / 归档历史治理提示、批量审核 / 置顶 / 加精与批量结果明细，并打通插件审计跳转闭环。插件市场、插件包上传、远程安装、Go 动态加载和第三方代码执行仍不属于当前实现。

下一阶段进入 `v1.4.x / P1` 插件平台增强：补权限矩阵、RBAC 配置草案、Hook 排障页和更完整的 schema 自动表单体验；仍不引入插件市场、远程安装或动态执行环境。
