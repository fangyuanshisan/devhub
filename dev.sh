...
# Load local development env (not committed). Keep this early so downstream `docker run -e`
# and `go run` pick up keys like DEVHUB_PLUGIN_CONFIG_KEY/KEY_ID for external_service token
# encryption, without requiring manual export every time.
...
DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST="${DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST:-}"
...
Options:
  --mysql           Start MySQL with docker compose and use CMS_STORE=mysql. This is the default.
  --memory          Use in-memory store. Data is reset on every backend restart.
...
Environment:
  PORT=8090
  CMS_STORE=mysql|memory
  SKIP_NPM_INSTALL=1
  FORCE_NPM_INSTALL=1
  FRONTEND_API_BASE=
  FRONTEND_SITE_URL=http://127.0.0.1:${PORT}
  DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST=
...
Examples:
  ...
  ./dev.sh stop
  ./dev.sh status
  DOCKER="sudo docker" ./dev.sh start
EOF
}
...
start_bootstrap_api_with_docker_go() {
  ...
  docker_cmd run -d --rm \
    ...
    -e "DB_NAME=$DB_NAME" \
    -e "DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST=$DEVHUB_EXTERNAL_SERVICE_HTTP_ALLOWLIST" \
    -e "DEVHUB_PLUGIN_CONFIG_KEYS=${DEVHUB_PLUGIN_CONFIG_KEYS:-}" \
    -e "DEVHUB_PLUGIN_CONFIG_KEY_ID=${DEVHUB_PLUGIN_CONFIG_KEY_ID:-}" \
    -e "DEVHUB_PLUGIN_CONFIG_KEY=${DEVHUB_PLUGIN_CONFIG_KEY:-}" \
    -e "DEVHUB_PLUGIN_CONFIG_OLD_KEYS=${DEVHUB_PLUGIN_CONFIG_OLD_KEYS:-}" \
    -e "GOPROXY=$GOPROXY" \
    -e "GOSUMDB=$GOSUMDB" \
