...
func (s *Server) deleteAdminPluginPackageUpload(c *gin.Context) {
	...
	s.auditStructured(c, "system", "plugin_package_upload.deleted", fmt.Sprintf("plugin-package-uploads#%s", uploadID), nil, gin.H{"status": "deleted"}, gin.H{"upload_id": uploadID, "items": res.Items, "actor": auditActor(c)})
	for _, item := range res.Items {
		if strings.HasPrefix(strings.TrimSpace(item.Path), "storage/plugins/quarantine/") || item.Status == domain.PluginPackageUploadStatusBlocked {
			s.auditStructured(c, "system", "plugin_package_quarantine.deleted", fmt.Sprintf("plugin-package-uploads#%s", uploadID), nil, gin.H{"status": "deleted"}, gin.H{"upload_id": uploadID, "path": item.Path, "actor": auditActor(c)})
		}
	}
	c.JSON(http.StatusOK, res)
}
...
func (s *Server) cleanupAdminPluginPackageUploads(c *gin.Context) {
	var req domain.PluginPackageCleanupRequest
	_ = c.ShouldBindJSON(&req)
	res, err := s.svc.CleanupPluginPackageUploads(req)
	if err != nil {
	...
	s.auditStructured(c, "system", "plugin_package_upload.cleanup", "plugin-package-uploads", nil, gin.H{"status": "ok"}, gin.H{"dry_run": res.DryRun, "will_delete_count": res.WillDeleteCount, "deleted_count": res.DeletedCount, "actor": auditActor(c)})
	if !res.DryRun {
		for _, item := range res.Items {
			if strings.HasPrefix(strings.TrimSpace(item.Path), "storage/plugins/quarantine/") || item.Status == domain.PluginPackageUploadStatusBlocked {
				s.auditStructured(c, "system", "plugin_package_quarantine.deleted", fmt.Sprintf("plugin-package-uploads#%s", item.ID), nil, gin.H{"status": "deleted"}, gin.H{"upload_id": item.ID, "path": item.Path, "actor": auditActor(c)})
			}
		}
	}
	c.JSON(http.StatusOK, res)
...
func (s *Server) promoteAdminPluginPackageDraft(c *gin.Context) {
	var req struct {
	...
	actor := auditActor(c)
	s.auditStructured(c, "system", "plugin.package.draft.promote.started", "plugin-packages", nil,
		gin.H{"status": "started"},
		gin.H{"operation": "plugin_package_draft_promote", "path": path, "force": req.Force, "actor": actor})
	res, err := s.svc.PromotePluginPackageDraft(path, req.Force)
	if err != nil {
		s.auditStructured(c, "system", "plugin.package.draft.promote.failed", "plugin-packages", nil,
			gin.H{"status": "failed"},
			mergeAuditMeta(gin.H{"operation": "plugin_package_draft_promote", "path": path, "actor": actor, "error": err.Error()}, auditAPIErrorFields(err)))
		failAPIError(c, err)
		return
	}
	s.auditStructured(c, "system", "plugin.package.draft.promoted", fmt.Sprintf("plugin-packages#%s", res.DryRun.Package.Code), nil,
		gin.H{"status": res.Status},
		gin.H{"operation": "plugin_package_draft_promote", "source_path": res.SourcePath, "package_path": res.PackagePath, "risk_level": res.DryRun.RiskReport.Level, "actor": actor})
	c.JSON(http.StatusOK, res)
}
...
func (s *Server) exportAdminPluginPackageTemplate(c *gin.Context) {
	var req domain.PluginPackageTemplateRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		fail(c, http.StatusBadRequest, err.Error())
		return
	...
	actor := auditActor(c)
	data, filename, preview, err := s.svc.ExportPluginPackageTemplateZip(req)
	if err != nil {
		s.auditStructured(c, "system", "plugin.package.template.export.failed", "plugins", nil,
			gin.H{"status": "failed"},
			mergeAuditMeta(gin.H{"operation": "plugin_package_template_export", "plugin_code": strings.TrimSpace(req.Code), "actor": actor, "error": err.Error()}, auditAPIErrorFields(err)))
		failAPIError(c, err)
		return
	}
	s.auditStructured(c, "system", "plugin.package.template.exported", fmt.Sprintf("plugin-packages#%s", preview.Code), nil,
		gin.H{"status": "ok"},
		gin.H{"operation": "plugin_package_template_export", "plugin_code": preview.Code, "plugin_type": preview.PluginType, "generated_files": preview.Files, "actor": actor})
...
func (s *Server) deleteAdminPluginPackageRepository(c *gin.Context) {
	path := strings.TrimSpace(c.Query("path"))
	if path == "" {
		var req struct {
			Path string `json:"path"`
	...
	res, err := s.svc.DeletePluginPackageRepositoryPackage(path)
	if err != nil {
		s.auditStructured(c, "system", "plugin.package.repository.delete.failed", "plugin-packages", nil, gin.H{"status": "failed"}, mergeAuditMeta(gin.H{"path": path, "actor": auditActor(c), "error": err.Error()}, auditAPIErrorFields(err)))
		failAPIError(c, err)
		return
	}
	s.auditStructured(c, "system", "plugin_package_repository.deleted", "plugin-packages", nil, gin.H{"status": "deleted"}, gin.H{"path": path, "items": res.Items, "actor": auditActor(c)})
	c.JSON(http.StatusOK, res)
}

func (s *Server) cleanupAdminPluginPackageRepository(c *gin.Context) {
	var req domain.PluginPackageCleanupRequest
	_ = c.ShouldBindJSON(&req)
	req.DryRun = false
	actor := auditActor(c)
	s.auditStructured(c, "system", "plugin.package.cleanup.started", "plugin-packages", nil, gin.H{"status": "started"}, pluginPackageCleanupAuditMeta(req, domain.PluginPackageCleanupResponse{}, actor))
	res, err := s.svc.CleanupPluginPackageRepository(req)
	if err != nil {
		s.auditStructured(c, "system", "plugin.package.cleanup.failed", "plugin-packages", nil, gin.H{"status": "failed"}, mergeAuditMeta(gin.H{"actor": actor, "scope": req.Scope, "error": err.Error()}, auditAPIErrorFields(err)))
		failAPIError(c, err)
		return
	}
	action := "plugin.package.cleanup.success"
	if res.FailedCount > 0 || len(res.Errors) > 0 || len(res.Warnings) > 0 {
		action = "plugin.package.cleanup.partial_failed"
	}
	s.auditStructured(c, "system", action, "plugin-packages", nil, gin.H{"status": "ok"}, pluginPackageCleanupAuditMeta(req, res, actor))
	for _, item := range res.Items {
		if !item.CanDelete {
			switch item.BlockedCode {
			case "plugin_package_repository_installed":
				s.auditStructured(c, "system", "plugin.package.cleanup.skipped_installed", "plugin-packages", nil, gin.H{"status": "skipped"}, gin.H{"path": item.Path, "plugin_code": item.PluginCode, "version": item.Version, "actor": actor})
			case "plugin_package_repository_enabled":
				s.auditStructured(c, "system", "plugin.package.cleanup.skipped_enabled", "plugin-packages", nil, gin.H{"status": "skipped"}, gin.H{"path": item.Path, "plugin_code": item.PluginCode, "version": item.Version, "actor": actor})
			case "plugin_package_cleanup_active_task":
				s.auditStructured(c, "system", "plugin.package.cleanup.skipped_active_task", "plugin-packages", nil, gin.H{"status": "skipped"}, gin.H{"path": item.Path, "plugin_code": item.PluginCode, "version": item.Version, "actor": actor})
			}
		}
	}
	c.JSON(http.StatusOK, res)
...
func (s *Server) previewAdminPluginPackageRepositoryCleanup(c *gin.Context) {
	var req domain.PluginPackageCleanupRequest
	_ = c.ShouldBindJSON(&req)
	req.DryRun = true
	req.ConfirmToken = ""
	res, err := s.svc.PreviewPluginPackageRepositoryCleanup(req)
	if err != nil {
		s.auditStructured(c, "system", "plugin.package.cleanup.failed", "plugin-packages", nil, gin.H{"status": "failed"}, mergeAuditMeta(gin.H{"actor": auditActor(c), "scope": req.Scope, "error": err.Error()}, auditAPIErrorFields(err)))
		failAPIError(c, err)
		return
	}
	s.auditStructured(c, "system", "plugin.package.cleanup.preview", "plugin-packages", nil, gin.H{"status": "preview"}, pluginPackageCleanupAuditMeta(req, res, auditActor(c)))
	c.JSON(http.StatusOK, res)
}

func pluginPackageCleanupAuditMeta(req domain.PluginPackageCleanupRequest, res domain.PluginPackageCleanupResponse, actor string) gin.H {
	packageIDs := make([]string, 0, len(res.Items))
	pluginCodes := make([]string, 0, len(res.Items))
	pathHashes := make([]string, 0, len(res.Items))
	for _, item := range res.Items {
		if item.ID != "" {
			packageIDs = append(packageIDs, item.ID)
		}
		if item.PluginCode != "" {
			pluginCodes = append(pluginCodes, item.PluginCode)
		}
		if item.Path != "" {
			pathHashes = append(pathHashes, shortSHA256(item.Path))
		}
	}
	return gin.H{
		"scope":               req.Scope,
		...
		"requested_by":        actor,
		"dry_run":             res.DryRun,
		"total_candidates":    res.WillDeleteCount + res.SkippedCount,
		"deleted_count":       res.DeletedCount,
		"skipped_count":       res.SkippedCount,
		"failed_count":        res.FailedCount,
		"will_delete_count":   res.WillDeleteCount,
		"will_free_bytes":     res.WillFreeBytes,
		"package_ids":         packageIDs,
		"plugin_codes":        pluginCodes,
		"storage_path_hashes": pathHashes,
	}
}
