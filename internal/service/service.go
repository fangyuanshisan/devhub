...
type Repository interface {
	...
	DeletePluginPackageUpload(uploadID string) error
	...
	SystemSetting(namespace, key string) (domain.SystemSetting, bool)
	SaveSystemSetting(record domain.SystemSetting) (domain.SystemSetting, error)
	...
	AppendSecretRef(record domain.SecretRefRecord) (domain.SecretRefRecord, error)
	SaveSecretRef(record domain.SecretRefRecord) (domain.SecretRefRecord, error)
	SecretRefByRef(ref string) (domain.SecretRefRecord, bool)
	SecretRefs(filter domain.SecretRefFilter) ([]domain.SecretRefRecord, int, error)
	TouchSecretRef(ref string, usedAt string) error
	SecretRefNamespaceCounts() (map[string]int, error)
...
func (s *Service) dispatchHook(event pluginregistry.HookEvent) error {
	...
	event.Ctx.RequestID = hookRequestID(event.Ctx.RequestID)
...
func hookRequestID(requestID string) string {
	requestID = strings.TrimSpace(requestID)
	if requestID != "" {
		return requestID
	}
	return "hookreq_" + randomHex(12)
}
...
// IsKnownContentType reports whether the content type is a built-in type or is
// declared by an installed manifest plugin.
func (s *Service) IsKnownContentType(contentType string) bool {
	_, ok := s.contentTypeDefinitionByType(contentType)
	return ok
}
...
func (s *Service) InjectFailedPluginMigrationForTest(pluginCode, migrationName, errorMessage, executor string) (domain.PluginMigration, error) {
	...
	defs := s.pluginMigrationDefinitions(pluginCode)
...
func (s *Service) pluginMigrationDefinitions(pluginCode string) []domain.PluginMigrationDefinition {
	pluginCode = strings.TrimSpace(pluginCode)
	if pluginCode == "" {
		return nil
	}
	if defs := pluginregistry.MigrationDefinitions(pluginCode); len(defs) > 0 {
		return defs
	}
	plugin, ok := s.PluginByCode(pluginCode)
	if !ok || plugin.Code == "" {
	...
	items := append([]domain.PluginMigrationDefinition(nil), plugin.Migrations...)
	for i := range items {
		if items[i].PluginCode == "" {
			items[i].PluginCode = plugin.Code
		}
		if items[i].MigrationVersion == "" {
			items[i].MigrationVersion = firstNonBlank(plugin.Version, "1.0.0")
		}
...
func (s *Service) PluginHealth(code string) (domain.PluginHealth, error) {
	...
	if cfg, ok := s.repo.PluginExternalServiceConfig(code); ok {
		...
		extSummary := s.externalServiceHealthExecutionSummary(code)
		health.ExternalServiceFailureCount24h = extSummary.FailureCount24h
		health.ExternalServiceFailureCount7d = extSummary.FailureCount7d
		health.ExternalServiceRecentSuccessExecutionID = extSummary.RecentSuccessExecutionID
		health.ExternalServiceRecentFailureExecutionID = extSummary.RecentFailureExecutionID
	...
	if err == nil {
		for _, stat := range stats {
			failureRecovered := externalServiceRecoveredAfterHookFailure(health, stat)
			if !failureRecovered {
				health.HookFailureCount += stat.FailureCount
			}
			if stat.Blocking && stat.FailureCount > 0 && !failureRecovered {
				health.HookStatus = "hook_error"
			} else if stat.FailureCount > 0 && !failureRecovered && health.HookStatus != "hook_error" {
				health.HookStatus = "hook_warning"
			}
			if stat.LastError != "" && !failureRecovered {
...
type externalServiceHealthExecutionSummary struct {
	FailureCount24h          int
	FailureCount7d           int
	RecentSuccessExecutionID string
	RecentFailureExecutionID string
}

func (s *Service) externalServiceHealthExecutionSummary(code string) externalServiceHealthExecutionSummary {
	if s == nil || s.repo == nil {
		return externalServiceHealthExecutionSummary{}
	}
	now := time.Now()
	failed := false
	_, failures24h, _ := s.repo.HookExecutionsByFilter(domain.HookExecutionFilter{
		PluginCode:  code,
	...
	_, failures7d, _ := s.repo.HookExecutionsByFilter(domain.HookExecutionFilter{
		PluginCode:  code,
		ServiceType: externalServiceTypeExternal,
		Success:     &failed,
	...
	success := true
	successRows, _, _ := s.repo.HookExecutionsByFilter(domain.HookExecutionFilter{
		PluginCode:  code,
		ServiceType: externalServiceTypeExternal,
		Success:     &success,
		Page:        1,
	...
	failureRows, _, _ := s.repo.HookExecutionsByFilter(domain.HookExecutionFilter{
		PluginCode:  code,
		ServiceType: externalServiceTypeExternal,
		Success:     &failed,
		Page:        1,
	...
	return externalServiceHealthExecutionSummary{
		...
		FailureCount7d:           failures7d,
		RecentSuccessExecutionID: firstExternalServiceExecutionID(successRows),
		RecentFailureExecutionID: firstExternalServiceExecutionID(failureRows),
	}
}

func firstExternalServiceExecutionID(rows []domain.HookExecution) string {
	for _, row := range rows {
		id := strings.TrimSpace(externalServiceExecutionIDFromRecord(row))
		if id != "" {
			return id
		}
		if row.ID > 0 {
			return fmt.Sprintf("record#%d", row.ID)
		}
	}
	return ""
}

func externalServiceRecoveredAfterHookFailure(health domain.PluginHealth, stat domain.HookStats) bool {
	if stat.FailureCount <= 0 || stat.Blocking {
		return false
	}
	if strings.TrimSpace(health.ExternalServiceHealthStatus) != externalServiceHealthHealthy {
		return false
	}
	lastSuccess := strings.TrimSpace(health.ExternalServiceLastSuccessAt)
	lastFailed := strings.TrimSpace(stat.LastFailedAt)
