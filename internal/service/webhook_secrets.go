...
func (s *Service) ListPluginWebhookSecrets(filter domain.PluginWebhookSecretFilter) (domain.PluginWebhookSecretListResponse, error) {
	...
	f := filter.Normalize()
	// Never expose secret material or derived hashes in list responses.
	_ = s.MarkExpiredPluginWebhookSecrets(time.Now(), 50)
	for i := range items {
		items[i] = sanitizePluginWebhookSecret(items[i])
	}
...
func (s *Service) GetPluginWebhookSecret(id int64) (domain.PluginWebhookSecret, error) {
	...
	_ = s.MarkExpiredPluginWebhookSecrets(time.Now(), 50)
	return sanitizePluginWebhookSecret(it), nil
}
...
DevHub 只能从启动环境变量读取该密钥，后台不会保存或生成 root key；修改后需要重启服务生效。
...
`)).
	...
	// Return once: strip stored secret material/hash from record, but include plaintext.
	saved = sanitizePluginWebhookSecret(saved)
...
`)).
	...
	// Return once: strip stored secret material/hash from record, return plaintext.
	savedNew = sanitizePluginWebhookSecret(savedNew)
...
func (s *Service) DisablePluginWebhookSecret(operator WebhookSecretOperator, id int64) (domain.PluginWebhookSecret, error) {
	...
	return sanitizePluginWebhookSecret(out), nil
}
...
func sanitizePluginWebhookSecret(record domain.PluginWebhookSecret) domain.PluginWebhookSecret {
