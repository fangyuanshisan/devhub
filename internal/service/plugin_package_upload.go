...
func (s *Service) DeletePluginPackageUpload(uploadID string) (domain.PluginPackageUploadDetailResponse, error) {
	record, err := s.getPluginPackageUploadRecord(uploadID)
	if err != nil {
		return domain.PluginPackageUploadDetailResponse{}, err
	}
	if record.Status == domain.PluginPackageUploadStatusDeleted {
		return domain.PluginPackageUploadDetailResponse{}, domain.NewPluginError("plugin_package_upload_already_deleted", "上传包已删除").
			WithStatus(400).WithDetail("upload_id", record.UploadID)
	}
	if record.Status == domain.PluginPackageUploadStatusApprovalPending || record.Status == domain.PluginPackageUploadStatusInstallApprovalPending {
		return domain.PluginPackageUploadDetailResponse{}, domain.NewPluginError("plugin_package_upload_status_conflict", "审批中的上传包不能直接删除").
			WithStatus(409).WithDetail("status", record.Status).WithSuggestion("请先取消审批或拒绝审批，再删除上传包。")
	}
	_ = removeUploadRecordFiles(record)
	record.Status = domain.PluginPackageUploadStatusDeleted
	record.DeletedAt = Now()
	record, _ = s.repo.SavePluginPackageUpload(record)
	return s.GetPluginPackageUploadDetail(record.UploadID)
}

func (s *Service) CleanupPluginPackageUploads() (domain.PluginPackageUploadCleanupResponse, error) {
	...
	for page := 1; ; page++ {
		...
		if err != nil {
			return domain.PluginPackageUploadCleanupResponse{}, err
		}
	...
	now := timeNow()
	res := domain.PluginPackageUploadCleanupResponse{Scanned: len(items)}
	for _, record := range items {
		...
		if uploadStatusIn(record.Status, domain.PluginPackageUploadStatusExpired, domain.PluginPackageUploadStatusDeleted, domain.PluginPackageUploadStatusCanceled, domain.PluginPackageUploadStatusFailed) {
...
func uploadActions(record domain.PluginPackageUploadRecord) []domain.PluginPackageUploadAction {
	...
	canDelete := !uploadStatusIn(record.Status, domain.PluginPackageUploadStatusDeleted, domain.PluginPackageUploadStatusApprovalPending, domain.PluginPackageUploadStatusInstallApprovalPending)
	return []domain.PluginPackageUploadAction{
		...
		action("delete", canDelete, disabledReason(canDelete, "plugin_package_upload_status_conflict"), disabledText(canDelete, "审批中的上传包不能直接删除")),
		action("cleanup", uploadStatusIn(record.Status, domain.PluginPackageUploadStatusExpired, domain.PluginPackageUploadStatusDeleted, domain.PluginPackageUploadStatusCanceled, domain.PluginPackageUploadStatusFailed), "", ""),
	}
}
