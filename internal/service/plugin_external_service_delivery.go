...
func (s *Service) enqueueExternalServiceHookDeliveries(event pluginregistry.HookEvent) {
	...
	for _, target := range s.externalServiceHookTargets(event) {
		s.enqueueExternalServiceHookDelivery(target.event, target.plugin, target.hook)
	}
}

type externalServiceHookTarget struct {
	event  pluginregistry.HookEvent
	plugin domain.Plugin
	hook   domain.HookDefinition
}

func (s *Service) externalServiceHookTargets(event pluginregistry.HookEvent) []externalServiceHookTarget {
	sourceCode := strings.TrimSpace(event.Ctx.PluginCode)
	seen := map[string]bool{}
	targets := []externalServiceHookTarget{}

	addPlugin := func(plugin domain.Plugin, dispatchEvent pluginregistry.HookEvent) {
		if strings.TrimSpace(plugin.Code) == "" {
			return
		}
		for _, hook := range plugin.Hooks {
			if !externalServiceHookMatches(dispatchEvent, hook) {
				continue
			}
			key := plugin.Code + "\x00" + hook.Name + "\x00" + normalizeExternalServiceHookPath(hook.Path)
			if seen[key] {
				continue
			}
			seen[key] = true
			targets = append(targets, externalServiceHookTarget{event: dispatchEvent, plugin: plugin, hook: hook})
		}
	}

	if sourceCode != "" && sourceCode != pluginregistry.CoreCode {
		if plugin, ok := s.PluginByCode(sourceCode); ok && plugin.Code != "" {
			addPlugin(plugin, event)
		}
	}

	for _, plugin := range s.repo.Plugins() {
		if !externalServicePluginSubscribes(plugin, event.Name) {
			continue
		}
		dispatchEvent := externalServiceSubscriberEvent(event, plugin.Code)
		addPlugin(plugin, dispatchEvent)
	}
...
func externalServicePluginSubscribes(plugin domain.Plugin, hookName string) bool {
	...
	for _, subscribed := range plugin.ExternalService.SubscribedHooks {
		if strings.TrimSpace(subscribed) == hookName {
			return true
		}
	}
	return false
...
func externalServiceSubscriberEvent(event pluginregistry.HookEvent, subscriberCode string) pluginregistry.HookEvent {
	out := event
	out.Ctx = event.Ctx
	out.Ctx.PluginCode = strings.TrimSpace(subscriberCode)
	metadata := map[string]any{}
	for k, v := range event.Ctx.Metadata {
		metadata[k] = v
	}
	metadata["source_plugin_code"] = firstNonEmpty(strings.TrimSpace(event.Ctx.PluginCode), pluginregistry.CoreCode)
	metadata["source_hook_name"] = strings.TrimSpace(event.Name)
	out.Ctx.Metadata = metadata
	return out
}
...
func (s *Service) performExternalServiceHookAttempt(event pluginregistry.HookEvent, plugin domain.Plugin, hook domain.HookDefinition, cfg domain.PluginExternalServiceConfig, executionID, eventID, idempotencyKey string) externalServiceHookExecutionRecord {
	...
	if normalizeExternalServiceAuthType(cfg.AuthType) == externalServiceAuthBearer {
		...
		if domain.IsSecretRef(cfg.TokenRef) {
			...
			if rerr != nil {
				...
				if apiErr, ok := rerr.(*domain.APIError); ok {
					...
					case "secret_ref_disabled":
						code = "secret_ref_disabled"
					case "secret_ref_revoked":
						code = "secret_ref_revoked"
			...
			plain, tokenErr := decryptExternalServiceToken(cfg.TokenCiphertext)
			if tokenErr != nil || strings.TrimSpace(plain) == "" {
				code = externalServiceReasonTokenInvalid
	...
	if err != nil {
		if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			...
			base.ErrorCode = "network_timeout"
			base.ErrorMessage = externalServiceDiagnosticMessage(cfg.EndpointURL, base.ErrorCode, err, 0, "")
			return base
		}
		diag := externalServiceNetworkDiagnostic(cfg.EndpointURL, err, ctx.Err())
		base.Status = externalServiceHookStatusFailed
		if diag.Status == "timeout" {
			base.Status = externalServiceHookStatusTimeout
		}
		base.ErrorCode = diag.Code
		base.ErrorMessage = externalServiceDiagnosticMessage(cfg.EndpointURL, diag.Code, err, 0, "")
		return base
	}
	...
	base.Status = externalServiceHookStatusFailed
	base.ErrorCode = "http_status_failed"
	...
	if base.ResponseBodyExcerpt == "" {
		base.ResponseBodyExcerpt = "HTTP " + strconv.Itoa(resp.StatusCode)
	}
	return base
}
...
func (s *Service) appendExternalServiceHookExecution(event pluginregistry.HookEvent, plugin domain.Plugin, hook domain.HookDefinition, cfg domain.PluginExternalServiceConfig, rec externalServiceHookExecutionRecord) domain.HookExecution {
	...
	if strings.TrimSpace(rec.ErrorCode) != "" {
		metadata["diagnostic_failure_type"] = externalServiceFailureType(rec.ErrorCode)
		metadata["diagnostic_suggestion"] = externalServiceDiagnosticSuggestion(cfg.EndpointURL, rec.ErrorCode, nil)
		metadata["diagnostics"] = []domain.PluginExternalServiceDiagnostic{
			externalServiceEndpointDiagnostic(cfg.EndpointURL, rec.ErrorCode, nil),
		}
	}
...
func externalServiceReasonText(reason string) string {
	...
	case "secret_ref_not_found":
		return "external_service token_ref 不存在"
	case "secret_ref_disabled":
		return "external_service token_ref 已停用"
	case "secret_ref_revoked":
		return "external_service token_ref 已吊销"
...
func (s *Service) ManualRetryExternalServiceHookExecution(operator PluginExternalServiceOperator, pluginCode string, executionRecordID int64) (domain.PluginExternalServiceManualRetryResponse, error) {
	...
	if !hasConfig || !cfg.Enabled {
		return domain.PluginExternalServiceManualRetryResponse{}, domain.NewPluginError("plugin_external_service_disabled", "external_service 已停用，不能手动重试").
...
func hookEventFromExecution(source domain.HookExecution, pluginCode string) pluginregistry.HookEvent {
	event := pluginregistry.HookEvent{
		...
		Ctx: pluginregistry.HookContext{
			...
			RequestID:   hookRequestID(source.RequestID),
			PluginCode:  pluginCode,
