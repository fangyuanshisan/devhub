...
func TestPluginPackageUploadLifecycle_DeletePromotedBlockedAndCleanup(t *testing.T) {
	_ = cleanupUploadStorage(t)
	root := mustProjectRoot(t)
	...
	if _, err := svc.DeletePluginPackageUpload(res.UploadID); apiCodeForUpload(err) != "plugin_package_upload_delete_not_allowed" {
		t.Fatalf("promoted upload record must be kept for traceability, got %v", err)
	}
	if _, err := os.Stat(filepath.Join(target, "manifest.json")); err != nil {
		t.Fatalf("promoted repository package must remain: %v", err)
	}
	...
	preview, err := svc.CleanupPluginPackageUploads(domain.PluginPackageCleanupRequest{DryRun: true, Statuses: []string{"expired"}})
	if err != nil {
		t.Fatalf("CleanupPluginPackageUploads dry-run: %v", err)
	}
	if preview.WillDeleteCount == 0 || preview.ConfirmToken == "" {
		t.Fatalf("expected dry-run cleanup plan: %#v", preview)
	}
	cleaned, err := svc.CleanupPluginPackageUploads(domain.PluginPackageCleanupRequest{Statuses: []string{"expired"}, ConfirmToken: preview.ConfirmToken})
	if err != nil {
