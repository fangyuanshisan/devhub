...
func TestMySQLStorePluginPlatformConsistency(t *testing.T) {
	...
	t.Run("failed migration blocks enable and retry restores readiness", func(t *testing.T) {
		...
		if _, err := svc.SetPluginStatus("qa", pluginregistry.StatusEnabled); err == nil || !isMySQLMigrationBlockedError(err) {
			t.Fatalf("expected failed migration to block global enable, got %v", err)
		}
		if _, err := svc.SetCommunityPluginStatus(1, "qa", pluginregistry.StatusEnabled); err == nil || !isMySQLMigrationBlockedError(err) {
...
func isMySQLMigrationBlockedError(err error) bool {
	if err == nil {
		return false
	}
	message := err.Error()
	return strings.Contains(message, "失败迁移") || strings.Contains(message, "迁移失败")
}
