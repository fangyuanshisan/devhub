...
func pluginPackageAllowedRoots(workdir string) []string {
	roots := []string{
		...
		filepath.Join(workdir, "storage", "plugins", "drafts"),
...
func NormalizePluginPackagePath(input string) (abs string, clean string, err error) {
	...
	if !isUnderAllowedRoots(abs, roots) {
		return "", "", domain.NewPluginError("plugin_package_path_invalid", "插件包路径不在允许目录内").
			...
			WithSuggestion("请把插件包放到 examples/plugins、plugins-local、storage/plugins/drafts、storage/plugins/packages、storage/plugins/exports、storage/plugins/staging、storage/plugins/quarantine、.devhub/plugins 目录下后重试。")
	}
