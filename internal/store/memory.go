...
type MemoryStore struct {
	...
	systemSettings         map[string]domain.SystemSetting
	...
	secretRefs             []domain.SecretRefRecord
	secretRefsByRef        map[string]*domain.SecretRefRecord
}
...
func (s *MemoryStore) SystemSetting(namespace, key string) (domain.SystemSetting, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	it, ok := s.systemSettings[systemSettingMapKey(namespace, key)]
	return it, ok
}

func (s *MemoryStore) SaveSystemSetting(record domain.SystemSetting) (domain.SystemSetting, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.systemSettings[systemSettingMapKey(record.Namespace, record.Key)] = record
	return record, nil
}

func systemSettingMapKey(namespace, key string) string {
	return strings.TrimSpace(namespace) + "\x00" + strings.TrimSpace(key)
}

// ===== Core SecretCenter (v1.8.4-S14) =====

func (s *MemoryStore) AppendSecretRef(record domain.SecretRefRecord) (domain.SecretRefRecord, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.rebuildSecretRefIndexLocked()
	if _, ok := s.secretRefsByRef[record.Ref]; ok {
		return domain.SecretRefRecord{}, errors.New("secret_ref 已存在")
	}
	record.ID = s.nextSecretRefID
	s.nextSecretRefID++
	if record.CreatedAt == "" {
		record.CreatedAt = now
	}
	record.UpdatedAt = now
	cp := record
	s.secretRefs = append(s.secretRefs, cp)
	s.rebuildSecretRefIndexLocked()
	return cp, nil
}

func (s *MemoryStore) SaveSecretRef(record domain.SecretRefRecord) (domain.SecretRefRecord, error) {
	record.Ref = strings.TrimSpace(record.Ref)
	if record.Ref == "" {
		ref, err := domain.BuildSecretRef(record.Namespace, record.Name)
		if err != nil {
	...
	defer s.mu.Unlock()
	s.rebuildSecretRefIndexLocked()
	if existing, ok := s.secretRefsByRef[record.Ref]; ok && existing != nil {
		// preserve immutable fields
		record.ID = existing.ID
		if record.CreatedAt == "" {
			record.CreatedAt = existing.CreatedAt
		}
		// preserve usage stats if caller didn't set them
...
func (s *MemoryStore) rebuildSecretRefIndexLocked() {
	if s.secretRefsByRef == nil {
		s.secretRefsByRef = map[string]*domain.SecretRefRecord{}
	}
	clear(s.secretRefsByRef)
	for i := range s.secretRefs {
		ref := strings.TrimSpace(s.secretRefs[i].Ref)
		if ref == "" {
			continue
		}
		s.secretRefsByRef[ref] = &s.secretRefs[i]
	}
}

func (s *MemoryStore) SecretRefByRef(ref string) (domain.SecretRefRecord, bool) {
...
func (s *MemoryStore) SecretRefs(filter domain.SecretRefFilter) ([]domain.SecretRefRecord, int, error) {
	f := filter.Normalize()
	ns := strings.TrimSpace(filter.Namespace)
	s.mu.RLock()
	defer s.mu.RUnlock()
	all := make([]domain.SecretRefRecord, 0)
	for i := len(s.secretRefs) - 1; i >= 0; i-- {
		it := s.secretRefs[i]
	...
	if offset >= total {
		return []domain.SecretRefRecord{}, total, nil
	}
	end := offset + f.PageSize
	if end > total {
		end = total
	}
	out := make([]domain.SecretRefRecord, 0, end-offset)
	out = append(out, all[offset:end]...)
	return out, total, nil
}

func (s *MemoryStore) TouchSecretRef(ref string, usedAt string) error {
	ref = strings.TrimSpace(ref)
	if ref == "" {
	...
	rec := s.secretRefsByRef[ref]
	if rec == nil {
		return errors.New("secret_ref 不存在")
	}
	rec.LastUsedAt = strings.TrimSpace(usedAt)
	rec.UsageCount++
	rec.UpdatedAt = Now()
	return nil
}

func (s *MemoryStore) SecretRefNamespaceCounts() (map[string]int, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	out := map[string]int{}
	for _, it := range s.secretRefs {
...
func (s *MemoryStore) seed() {
	...
	s.roles[2] = domain.AdminRole{ID: 2, Name: "站点管理员", Builtin: true, Description: "负责授权子站的内容和举报治理", Permissions: []string{"dashboard.read", "site.read", "site.write", "board.read", "board.write", "post.read", "post.create", "post.update", "post.delete", "topic.moderate", "comment.read", "comment.moderate", "report.read", "report.handle", "moderator.read", "notification.write", "log.read", "plugin.read", "qa.question.create", "qa.question.audit", "docs.document.create", "docs.document.audit", "docs.space.manage", "wiki.page.create", "wiki.page.audit", "wiki.page.version.rollback", "projects.project.create", "projects.project.audit", "jobs.job.create", "jobs.job.audit", "ai_works.work.create", "ai_works.work.audit", "plugin_a7b0cc04.link.create", "plugin_a7b0cc04.link.audit"}, UserCount: 1}
	s.roles[3] = domain.AdminRole{ID: 3, Name: "内容审核员", Builtin: true, Description: "负责授权子站的内容审核和评论治理", Permissions: []string{"dashboard.read", "post.read", "post.update", "topic.moderate", "comment.read", "comment.moderate", "report.read", "report.handle", "plugin.read", "qa.question.audit", "docs.document.audit", "wiki.page.audit", "plugin_a7b0cc04.link.audit"}, UserCount: 1}
...
func (s *MemoryStore) memoryAuthUserLocked(userID int64, identity string) domain.AuthUser {
	...
	if identity == "user" {
		...
		perms = []string{"post.create", "core.topic.create", "comment.read", "qa.question.create", "qa.answer.create", "docs.document.create", "wiki.page.create", "projects.project.create", "jobs.job.create", "ai_works.work.create", "plugin_a7b0cc04.link.create"}
	}
	return domain.AuthUser{
		ID:          u.ID,
		Username:    u.Username,
...
func (s *MemoryStore) DeletePluginPackageUpload(uploadID string) error {
	uploadID = strings.TrimSpace(uploadID)
	s.mu.Lock()
	defer s.mu.Unlock()
	for i := range s.packageUploads {
		if s.packageUploads[i].UploadID != uploadID {
			continue
		}
		s.packageUploads = append(s.packageUploads[:i], s.packageUploads[i+1:]...)
		return nil
	}
	return nil
}
