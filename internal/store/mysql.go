...
func (s *MySQLStore) SystemSetting(namespace, key string) (domain.SystemSetting, bool) {
	namespace = strings.TrimSpace(namespace)
	key = strings.TrimSpace(key)
	if namespace == "" || key == "" {
		return domain.SystemSetting{}, false
	}
	var it domain.SystemSetting
	err := s.db.QueryRow(`SELECT namespace,setting_key,COALESCE(CAST(value_json AS CHAR),''),DATE_FORMAT(updated_at,'%Y-%m-%d %H:%i:%s') FROM system_settings WHERE namespace=? AND setting_key=? LIMIT 1`, namespace, key).
		Scan(&it.Namespace, &it.Key, &it.Value, &it.UpdatedAt)
	return it, err == nil
}

func (s *MySQLStore) SaveSystemSetting(record domain.SystemSetting) (domain.SystemSetting, error) {
	record.Namespace = strings.TrimSpace(record.Namespace)
	record.Key = strings.TrimSpace(record.Key)
	if record.Namespace == "" || record.Key == "" {
		return domain.SystemSetting{}, errors.New("system setting namespace/key 不能为空")
	}
	_, err := s.db.Exec(`INSERT INTO system_settings (namespace,setting_key,value_json,created_at,updated_at) VALUES (?,?,?,NOW(),NOW())
		ON DUPLICATE KEY UPDATE value_json=VALUES(value_json),updated_at=NOW()`, record.Namespace, record.Key, mysqlJSONArg(record.Value))
	if err != nil {
		return domain.SystemSetting{}, err
	}
	out, _ := s.SystemSetting(record.Namespace, record.Key)
	return out, nil
}

// ===== Core SecretCenter (v1.8.4-S14) =====

func (s *MySQLStore) SecretRefByRef(ref string) (domain.SecretRefRecord, bool) {
	ref = strings.TrimSpace(ref)
	if ref == "" {
		return domain.SecretRefRecord{}, false
	}
	var it domain.SecretRefRecord
	err := s.db.QueryRow(`SELECT id,ref,namespace,name,COALESCE(key_id,''),COALESCE(encrypted_value,''),COALESCE(status,''),COALESCE(description,''),
		COALESCE(DATE_FORMAT(last_used_at,'%Y-%m-%d %H:%i:%s'),''),COALESCE(usage_count,0),COALESCE(DATE_FORMAT(rotated_at,'%Y-%m-%d %H:%i:%s'),''),
		COALESCE(created_by,0),COALESCE(updated_by,0),
		DATE_FORMAT(created_at,'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(updated_at,'%Y-%m-%d %H:%i:%s')
		FROM secret_refs WHERE ref=? LIMIT 1`, ref).
		Scan(&it.ID, &it.Ref, &it.Namespace, &it.Name, &it.KeyID, &it.EncryptedValue, &it.Status, &it.Description, &it.LastUsedAt, &it.UsageCount, &it.RotatedAt, &it.CreatedBy, &it.UpdatedBy, &it.CreatedAt, &it.UpdatedAt)
	if err != nil {
		return domain.SecretRefRecord{}, false
	}
	return it, true
...
func (s *MySQLStore) AppendSecretRef(record domain.SecretRefRecord) (domain.SecretRefRecord, error) {
	record.Ref = strings.TrimSpace(record.Ref)
	record.Namespace = strings.TrimSpace(record.Namespace)
	record.Name = strings.TrimSpace(record.Name)
	if record.Ref == "" {
		ref, err := domain.BuildSecretRef(record.Namespace, record.Name)
		if err != nil {
			return domain.SecretRefRecord{}, err
		}
		record.Ref = ref
	}
	ns, nm, err := domain.ParseSecretRef(record.Ref)
	if err != nil {
		return domain.SecretRefRecord{}, err
	}
	record.Namespace, record.Name = ns, nm
	if record.Status == "" {
		record.Status = domain.SecretRefStatusActive
	}
	_, err = s.db.Exec(`INSERT INTO secret_refs
		(ref,namespace,name,key_id,encrypted_value,status,description,last_used_at,usage_count,rotated_at,created_by,updated_by,created_at,updated_at)
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())`,
		record.Ref, record.Namespace, record.Name, record.KeyID, record.EncryptedValue, record.Status, record.Description,
		nullableTimeString(record.LastUsedAt), record.UsageCount, nullableTimeString(record.RotatedAt),
		record.CreatedBy, record.UpdatedBy)
	if err != nil {
		return domain.SecretRefRecord{}, err
	}
	out, ok := s.SecretRefByRef(record.Ref)
	if !ok {
		return record, nil
	}
	return out, nil
...
func (s *MySQLStore) SaveSecretRef(record domain.SecretRefRecord) (domain.SecretRefRecord, error) {
	record.Ref = strings.TrimSpace(record.Ref)
	record.Namespace = strings.TrimSpace(record.Namespace)
	record.Name = strings.TrimSpace(record.Name)
	if record.Ref == "" {
...
func (s *MySQLStore) SecretRefs(filter domain.SecretRefFilter) ([]domain.SecretRefRecord, int, error) {
	f := filter.Normalize()
	where := "WHERE 1=1"
	args := []any{}
	if ns := strings.TrimSpace(filter.Namespace); ns != "" {
		where += " AND namespace=?"
		args = append(args, ns)
	}
	total := 0
	if err := s.db.QueryRow("SELECT COUNT(*) FROM secret_refs "+where, args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	offset := (f.Page - 1) * f.PageSize
	queryArgs := append(append([]any{}, args...), f.PageSize, offset)
	rows, err := s.db.Query(`SELECT id,ref,namespace,name,COALESCE(key_id,''),COALESCE(status,''),COALESCE(description,''),
		COALESCE(DATE_FORMAT(last_used_at,'%Y-%m-%d %H:%i:%s'),''),COALESCE(usage_count,0),COALESCE(DATE_FORMAT(rotated_at,'%Y-%m-%d %H:%i:%s'),''),
		COALESCE(created_by,0),COALESCE(updated_by,0),
		DATE_FORMAT(created_at,'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(updated_at,'%Y-%m-%d %H:%i:%s')
		FROM secret_refs `+where+` ORDER BY id DESC LIMIT ? OFFSET ?`, queryArgs...)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	out := []domain.SecretRefRecord{}
	for rows.Next() {
		var it domain.SecretRefRecord
		if err := rows.Scan(&it.ID, &it.Ref, &it.Namespace, &it.Name, &it.KeyID, &it.Status, &it.Description, &it.LastUsedAt, &it.UsageCount, &it.RotatedAt, &it.CreatedBy, &it.UpdatedBy, &it.CreatedAt, &it.UpdatedAt); err == nil {
			// Never return encrypted_value in list.
			it.EncryptedValue = ""
			out = append(out, it)
		}
	}
...
func (s *MySQLStore) TouchSecretRef(ref string, usedAt string) error {
	ref = strings.TrimSpace(ref)
	if ref == "" {
		return errors.New("secret_ref 不能为空")
	}
	_, err := s.db.Exec(`UPDATE secret_refs SET last_used_at=?, usage_count=usage_count+1, updated_at=NOW() WHERE ref=?`, nullableTimeString(usedAt), ref)
	return err
}

func (s *MySQLStore) SecretRefNamespaceCounts() (map[string]int, error) {
	rows, err := s.db.Query(`SELECT namespace, COUNT(*) FROM secret_refs GROUP BY namespace`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *MySQLStore) seedAuthData() error {
	permissions := []struct {
		...
		{"plugin_a7b0cc04.link.create", "plugin_a7b0cc04", "link.create", "发布飞书链接"},
		{"plugin_a7b0cc04.link.edit", "plugin_a7b0cc04", "link.edit", "编辑飞书链接"},
		{"plugin_a7b0cc04.link.delete", "plugin_a7b0cc04", "link.delete", "删除飞书链接"},
		{"plugin_a7b0cc04.link.audit", "plugin_a7b0cc04", "link.audit", "审核飞书链接"},
	}
	...
	roles := []struct {
		...
		{2, "site_admin", "站点管理员", "管理被授权站点", []string{"dashboard.read", "site.read", "site.write", "board.read", "board.write", "post.read", "post.create", "post.update", "post.delete", "topic.moderate", "comment.read", "comment.moderate", "report.read", "report.handle", "moderator.read", "user.read", "setting.read", "notification.write", "log.read", "plugin.read", "qa.question.create", "qa.question.audit", "docs.document.create", "docs.document.audit", "docs.space.manage", "wiki.page.create", "wiki.page.audit", "wiki.page.version.rollback", "projects.project.create", "projects.project.audit", "jobs.job.create", "jobs.job.audit", "ai_works.work.create", "ai_works.work.audit", "plugin_a7b0cc04.link.create", "plugin_a7b0cc04.link.edit", "plugin_a7b0cc04.link.delete", "plugin_a7b0cc04.link.audit"}},
		{3, "editor", "编辑", "创建和编辑内容", []string{"dashboard.read", "post.read", "post.create", "post.update", "comment.read", "projects.project.create", "jobs.job.create", "ai_works.work.create", "plugin_a7b0cc04.link.create", "plugin_a7b0cc04.link.edit"}},
		{4, "moderator", "审核员", "审核内容和评论", []string{"dashboard.read", "post.read", "post.update", "topic.moderate", "comment.read", "comment.moderate", "report.read", "report.handle", "plugin.read", "qa.question.audit", "docs.document.audit", "wiki.page.audit", "projects.project.audit", "jobs.job.audit", "ai_works.work.audit", "plugin_a7b0cc04.link.audit"}},
		{5, "user", "普通用户", "前台登录用户", []string{"post.create", "comment.read", "qa.question.create", "qa.answer.create", "docs.document.create", "wiki.page.create", "projects.project.create", "jobs.job.create", "ai_works.work.create", "plugin_a7b0cc04.link.create"}},
	}
	for _, r := range roles {
...
func (s *MySQLStore) DeletePluginPackageUpload(uploadID string) error {
	_, err := s.db.Exec(`DELETE FROM plugin_package_uploads WHERE upload_id=?`, strings.TrimSpace(uploadID))
	return err
}
