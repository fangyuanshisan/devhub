...
当前稳定版本：`v1.9.0`，主题为“官方插件生态稳定版”。DevHub v1.9.0 官方插件生态稳定版已完成发布归档与发布后 targeted smoke，P0=0，P1=0，P2=0，版本可以冻结。建议先提交当前文档归档，再创建 v1.9.0 annotated tag。v1.9.0 冻结后，项目进入下一阶段规划；不再向 v1.9.0 追加新功能。`v1.9.1` 作为小修 / 文档 / 测试 / 兼容性维护候选，`v1.10.0` 作为更大功能、治理和架构演进的规划阶段。当前仓库 `VERSION` 为最终版本来源。
...
- [docs/releases/v1.9.0.md](docs/releases/v1.9.0.md)
- [docs/releases/v1.8.4.md](docs/releases/v1.8.4.md)
...
- 官方插件与开发模板：当前提供生产化官方友情链接插件包 `examples/plugins/official_links/`、官方 external_service Webhook 样板 `examples/plugins/official_webhook_notify/`、[声明型插件开发者指南](docs/PLUGIN_DEVELOPER_GUIDE.md)、纯声明型内容插件模板 `examples/plugins/templates/declarative-content/` 和 external_service Webhook 插件模板 `examples/plugins/templates/external-service-webhook/`。建议先看指南，再复制模板；可用 `scripts/plugin-package-build.sh` / `scripts/plugin-package-check.sh` 本地生成与校验官方包和模板。官方包和模板都只包含声明、配置示例和迁移入口说明，不包含运行时代码、真实 secret、远程 iframe 或 blocking Hook。
- 后台创建插件模板：v1.9.0-S6 稳定化“初始化插件包 / 创建插件模板”表单，支持插件名称自动生成 code（小写字母、数字、下划线、中划线）、高级手动修改、插件类型动态字段、发布者 / 作者下拉、preview 文件树 / 权限 / 菜单 / migrations / conflicts，以及 `storage/plugins/drafts/{code}` draft 输出和 zip export。后台、CLI 和 export 共用 `PluginTemplateGenerator`，只生成声明型安全模板、`docs/*.md` 和 `migrations/001_init.sql`，不生成 `registry.example.go`、根目录 `001_schema.sql`、可执行代码、package scripts 或 blocking Hook 模板，不执行 SQL 或第三方代码。
- 插件包本地仓库清理：v1.8.4-S8 已在插件包治理 -> 本地仓库补齐测试包 / 未安装包批量清理。清理前会 preview，执行只清理未安装的 `e2e_` / `fixture_` / `test_` / `demo_` 测试包或 blocked / invalid / warning / promoted 未安装包；installed / enabled / active task 包会跳过。清理会删除 `storage/plugins/packages/` 对应目录和 promoted upload 本地记录，并写 `admin_logs`，不建议手工直接删数据库。
- 升级 dry-run UI：v1.9.0-S7 补强 `/admin-next/plugins/install` 的升级预览结果展示，按结构化 `diff_sections`、`impact_summary`、分区计划、`rollback_boundary`、`dependency_diff` 和 `frontend_mount_plan` 呈现；`blocked_items` / `warnings` 前置，warning 需要显式确认，blocked 不能继续且不能被 confirm 绕过。该 UI 只消费既有后端结构化结果，不改变升级语义。
- 生产备份与升级演练：v1.8.4-S5 已把插件安装 / 升级前的生产备份清单、install / upgrade dry-run 检查、`safe / warning / blocked` 处理、`failure_stage` / `failure_reason` 恢复建议、PluginRegistry reload 失败路径、配置与加密 key 恢复、本地插件仓库恢复、MySQLStore 生产建议和 MemoryStore 限制整理到 [备份与回滚文档](docs/BACKUP_AND_ROLLBACK.md)。当前仍不承诺完整自动 rollback 或 migration down。
...
- 后台插件治理点击矩阵：v1.9.0-S8 已将 `scripts/check-admin-plugin-ia.sh` 收口为插件治理 5 个域 + 系统设置 / SecretCenter / HTTP Allowlist / 当前生效配置的一键浏览器回归入口，截图归档在 `.devhub/screenshots/plugin-ia`，覆盖 1024 / 1366 宽度、旧路由跳转、Tab 状态保留、详情抽屉、技术详情折叠和敏感信息扫描。
- Webhook 本地联调体验：v1.8.4-S11 已补齐 external_service 配置直达、Docker loopback 提示、HTTP allowlist 诊断、当前健康 / 历史失败拆分、重复安装引导和升级差异入口。DevHub 后端在 Docker 容器中时，`127.0.0.1` 指向容器自身；宿主机 receiver 建议使用 `host.docker.internal`、Docker host gateway 地址或宿主机局域网 IP。非 localhost HTTP endpoint 仍需显式 allowlist；v1.8.4-S16 起 allowlist 支持环境变量来源和后台受控配置来源，系统默认允许 `localhost` / `127.0.0.1` / `::1`，HTTPS 默认允许，后台配置只允许 exact `http://host[:port]`，不允许 wildcard、`0.0.0.0` 或 CIDR。
- v1.9.0-S1 已补齐 fresh standalone receiver 回归：`scripts/run-feishu-webhook-flow.sh` 支持 `DEVHUB_WEBHOOK_FLOW=full`，可覆盖 `feishu_link` success、500 failure、timeout 和 manual retry，并检查 `hook_executions`、`admin_logs`、`token_ref` 与敏感信息脱敏。
- SecretCenter 与当前生效配置：v1.8.4-S14 已新增 Core SecretCenter 最小闭环和 `secret://{namespace}/{name}` 引用规范，external_service bearer token 会保存为 `token_ref=secret://external_service/{plugin_code}/token`；API、后台、审计和执行记录不回显 token / secret / Authorization。v1.8.4-S19 起，系统设置 -> SecretCenter 以“敏感配置引用”方式展示 ref、来源、使用关系和影响预览，可区分 external_service、Webhook Secret、Callback Token 与测试数据；禁用 / 吊销会先预览影响，吊销要求完整 ref 强确认，轮换只跳转来源治理。v1.9.0-S2 补强详情抽屉与排障入口：展示 namespace、name/key、usage_type、source_type/source_id/source_code、created/updated by 和使用关系；未知来源不误导跳转，测试 / fixture / seed / demo 数据不开放危险操作；Secret 审计可按 ref、namespace/key、source_type/source_id/source_code 或 plugin_code 搜索。v1.9.0-S3 稳定当前生效配置页：分区展示基础运行信息、root key 只读状态、SecretCenter、HTTP Allowlist 来源与 endpoint origin 匹配、external_service 非敏感字段、`token_ref` 脱敏来源元数据、Webhook / Callback 安全摘要、quick links 和可复制脱敏 `diagnostic_text`；复制失败时提供手动复制文本框。脱敏诊断不包含 token / secret / Authorization / root key / `DEVHUB_PLUGIN_CONFIG_KEYS` env 示例 / encrypted_value / token_hash。`DEVHUB_PLUGIN_CONFIG_KEYS` 或兼容单 key 仍只能来自启动环境变量或外部 Secret 系统注入，后台不会保存、生成或修改 root key，也不能编辑 `DEVHUB_PLUGIN_CONFIG_KEYS`。
- 后台 UI 基础设计系统：v1.8.4-S20 已新增后台 design tokens 与通用组件，插件列表、插件详情、Webhook 治理、当前生效配置、SecretCenter、插件包治理入口和版本仓库入口已接入统一标题区、指标卡、状态标签和默认折叠技术详情。该轮只改后台 UI，不改变 API、插件生命周期、external_service 投递逻辑或 SecretCenter 安全模型，不引入新大型 UI 框架；详见 [后台 UI 基础设计规范](docs/ADMIN_UI_GUIDELINES.md)。
...
当前 `v1.9.0` 重点是官方插件生态稳定版，不是动态插件运行时版本。完整第三方代码执行、完整第三方运行模型、远程动态加载、任意远程 iframe、插件市场、远程在线安装、package scripts、自动安装、自动启用和 blocking Hook 仍未开放，详见 [插件系统路线图](docs/PLUGIN_SYSTEM_ROADMAP.md) 与 [v1.9.0 Release Notes](docs/releases/v1.9.0.md)。

当前范围和限制以 [v1.9.0 Release Notes](docs/releases/v1.9.0.md) 为准，长期滚动状态见 [项目进度](docs/PROJECT_PROGRESS.md)。

历史版本说明见 `docs/README.md` 的“历史版本归档”。

## Roadmap

- v1.9.0：已冻结为官方插件生态稳定版；官方插件生态、MySQLStore 生产模式、插件包治理、模板 `preview / generate / export + CLI`、upgrade dry-run UI、后台插件治理 IA、SecretCenter / `token_ref`、external_service non-blocking Webhook、HTTP Allowlist、Webhook Secret / Callback Token 一次性明文模型、敏感扫描归档、发布归档与 targeted smoke 均已收口。
- v1.9.1：维护候选，优先处理小修、文档口径巡检、测试脚本固化、兼容性补丁和发布后 smoke 经验沉淀。
- v1.10.0：规划阶段，面向更大的官方插件样例、治理能力、远程分发前置治理和完整第三方运行模型设计拆分；不把这些规划写作已实现。
- 安全边界继续保持：不执行第三方插件代码，不开放 Go plugin / JS sandbox / WASM / Lua / remote iframe / remote component / arbitrary remote JS / plugin marketplace / remote online install / blocking Hook / package scripts / auto-install / auto-enable。
...
脚本默认固定端口为 `8090`，默认使用 MySQL 开发库，适合保留插件安装信息、配置和治理记录。它会自动构建 Astro 前台和 Vue 后台；Node 构建优先使用本机 `npm`，没有本机 `npm` 时使用 Docker Node。Go 后端默认统一使用 Docker Go 镜像，当前用户无 Docker 权限时，脚本会自动尝试 `sudo docker`。
...
`dev.sh` 默认使用 MySQL 模式，插件安装信息、配置和治理记录会落到开发库并随 `devhub_mysql_data` volume 保留：

```bash
./dev.sh
```

如果只想临时体验、允许重启后清空数据，可以显式使用内存模式：
...
后台 Playwright E2E 推荐使用 Compose runner。`devhub` 服务会在容器内按当前源码构建临时后端并等待 `/api/v1/health`，`admin-e2e` 默认访问 `http://devhub:8090`：
