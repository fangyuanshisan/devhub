...
print_e2e_diagnostics() {
  local service="$1"
  local origin="${DEVHUB_E2E_ORIGIN:-http://devhub:8090}"

  echo
  echo "${C_YELLOW}E2E 诊断信息（已避免输出请求凭据）：${C_RESET}"
  echo "${C_GRAY}Playwright baseURL: ${origin}${C_RESET}"
  echo "${C_GRAY}docker compose ps -a:${C_RESET}"
  "${DC[@]}" ps -a || true
  echo "${C_GRAY}devhub 最近日志:${C_RESET}"
  "${DC[@]}" logs --no-color --tail=80 devhub || true

  if service_exists "$service"; then
    echo "${C_GRAY}${service} 容器内 devhub DNS / HTTP 探测:${C_RESET}"
    "${DC[@]}" run --rm --no-deps "$service" sh -lc '
      echo "DEVHUB_E2E_ORIGIN=${DEVHUB_E2E_ORIGIN:-http://devhub:8090}"
      if command -v getent >/dev/null 2>&1; then
        getent hosts devhub || true
      else
        echo "getent unavailable"
      fi
      if command -v wget >/dev/null 2>&1; then
        wget -S -O- "${DEVHUB_E2E_ORIGIN:-http://devhub:8090}/api/v1/health" 2>&1 | head -80 || true
      elif command -v curl >/dev/null 2>&1; then
        curl --noproxy "*" -i -sS "${DEVHUB_E2E_ORIGIN:-http://devhub:8090}/api/v1/health" | head -80 || true
      else
        echo "wget/curl unavailable"
      fi
    ' || true
  fi
}
...
run_e2e_default() {
  ...
  if ! run_step "${title} E2E" "${DC[@]}" run "${opts[@]}" "$service"; then
    print_e2e_diagnostics "$service"
    return 1
  fi
